...
int add_timer(long usec, timeout_handler handler)
{
	...
	if (first_timer == NULL) {
		...
		for (tmp=first_timer; tmp->next == NULL; tmp = tmp->next ) {
...
{
  if (debug_level)
    debug_printf("Adding the filedescriptor %d to the select()\n", fd);
...
/* initialize fds (for select) and return the highest numbered file
 * descriptor */
...
/* return timer with shortest timeout remaining */
...
void select_loop(void)
{
	...
	for (;;) {
		...
		/* do select and see how long it took */
