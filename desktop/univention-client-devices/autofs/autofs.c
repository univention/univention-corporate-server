 ...
 *	autofs tool based an a tool from the FUSE project
 *
...
static int autofs_statfs(struct fuse_statfs *fst);
...
struct fuse_statfs statfs_cache;
...
int __inline__ do_mount(void)
{
	...
	DEBUG("mounting\n");
	res =
	    mount(device, mountpoint, filesystem, mountoptions, mountdata);
	if (res != 0 && mountoptions == 0) {
...
static int autofs_statfs(struct fuse_statfs *fst)
{
	...
	pthread_mutex_lock(&statfs_mutex);
	memcpy(fst, &statfs_cache, sizeof(struct fuse_statfs));
	pthread_mutex_unlock(&statfs_mutex);
...
void usage(void)
{
	fprintf(stderr, "autofs dev mountpoint fs ro|rw mount-options\n");
	exit(1);
}
...
int main(int argc, char *argv[])
{
	...
	if (strcmp(argv[4], "ro") == 0) {
		mountoptions = MS_RDONLY | MS_NOSUID | MS_NODEV;
	...
	fuse_main(fuse_argc, fuse_argv, &autofs_oper);
	return 0;
}
