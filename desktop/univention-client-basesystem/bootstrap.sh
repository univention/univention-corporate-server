...
export packages="univention-config-registry python-support python-univention libnss-ldap libpam-ldap nfs-common heimdal-clients openssh-client gdm samba samba-common univention-x-core univention-fonts python2.4 bind9-host pam-runasroot ntpdate ntp less rdesktop fuse-utils univention-automount arts kcontrol dhcp3-client libpam-heimdal heimdal-clients cupsys univention-policy-tools libunivention-policy0 libldap2 ldap-utils python-dns kudzu-2.6 univention-gdm lynx locales univention-xnumlock univention-home-mounter rdate netcat fuse-utils smbfs univention-automount univention-client-devices-client univention-bootsplash univention-kde-setdirs console-data alsa-base alsa-utils"
...
chmod 755 "/sbin/start-stop-daemon"
	...
	if ! test -L /etc/univention/base.conf; then
		...
		for f in base.conf base-ldap.conf base-forced.conf ; do
	    	...
	    	touch /ramdisk/etc/univention/${f}.bak
