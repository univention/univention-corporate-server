...
class Component(object):
    ...
    def repositories(self, start, end, clean=False, for_mirror_list=False):
        # type: (UCS_Version, UCS_Version, bool, bool) -> Iterator[str]
        """
        ...
        for server, struct in self.versions(start, end, for_mirror_list):
            for struct.arch in struct.ARCHS:
                if current:  # FIXME: PMH make non-existing current component fatal
...
class UniventionUpdater(object):
    ...
    def get_next_version(self, version, components=[], errorsto='stderr'):
        ...
        for component in components:
            ...
            try:
                any(component.repositories(ver, ver))
    ...
    def release_update_temporary_sources_list(self, version):
        ...
        for comp in self.get_components():
            result += list(comp.repositories(version, version))
    ...
    def print_component_repositories(self, clean=False, start=None, end=None, for_mirror_list=False):
        ...
        for comp in self.get_components(only_localmirror_enabled=for_mirror_list):
            try:
                result += comp.repositories(start, end, clean=clean, for_mirror_list=for_mirror_list)
            except UpdaterException as ex:
                result.append("# Component %s: %s" % (comp.name, ex))
