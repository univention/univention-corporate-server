...
def main() -> None:
    ...
    with UpdaterLock():
        ...
        current_ucs_version = "%s-%s" % (configRegistry.get('version/version'), configRegistry.get('version/patchlevel'))
        options.version = UCS_Version(current_ucs_version)  # FIXME
        ...
        print(dedent(
            r"""
            The local repository has been prepared. The repository can be filled and
            updated using:
