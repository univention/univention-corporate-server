...
from mockups import (
    MAJOR, MINOR, PATCH, ARCH, ERRAT, PART, gen_releases,
)
...
class TestUniventionUpdater(object):
    ...
    def test_get_all_available_release_updates(self, ucr, u, http):
        ...
        uris = {
            ...
            '/univention-repository/%d.%d/maintained/component/%s/all/Packages.gz' % (MAJOR, MINOR + 1, 'a'): DATA,
            '/univention-repository/%d.%d/maintained/component/%s/amd64/Packages.gz' % (MAJOR, MINOR + 1, 'a'): DATA,
        }
        http(uris)
    ...
    def test_release_update_temporary_sources_list(self, ucr, u, http):
        ...
        uris = {
            'univention-repository/%d.%d/maintained/component/%s/%s/Packages.gz' % (MAJOR, MINOR + 1, 'a', 'all'): DATA,
            'univention-repository/%d.%d/maintained/component/%s/%s/Packages.gz' % (MAJOR, MINOR + 1, 'a', ARCH): DATA,
            'univention-repository/%d.%d/maintained/component/%s/%s/Packages.gz' % (MAJOR, MINOR + 1, 'b', 'all'): DATA,
            'univention-repository/%d.%d/maintained/component/%s/%s/Packages.gz' % (MAJOR, MINOR + 1, 'b', ARCH): DATA,
            RJSON: gen_releases([(MAJOR, MINOR, PATCH), (MAJOR, MINOR + 1, 0)])
        }
        http(uris)
        ...
        assert {
            ...
            'deb https://updates.software-univention.de/univention-repository/%d.%d/maintained/component/ %s/%s/' % (MAJOR, MINOR + 1, 'a', 'all'),
            'deb https://updates.software-univention.de/univention-repository/%d.%d/maintained/component/ %s/%s/' % (MAJOR, MINOR + 1, 'a', ARCH),
        } == set(tmp)
    ...
    def test_get_current_component_status_DISABLED(self, ucr, u):
    ...
    @pytest.mark.skip(reason='TBD: should the updater check credentials?')
    def test_get_current_component_status_PERMISSION(self, ucr, u):
        ...
        try:
            tmp = NamedTemporaryFile()
            tmp.write(b'deb http://host:port/prefix/0.0/maintained/component/ d/arch/')
            tmp.write(b'# credentials not accepted: d')
            tmp.flush()
            U.Component.FN_APTSOURCES = tmp.name
    ...
    def test_get_current_component_status_UNKNOWN(self, ucr, u):
    ...
    def test_get_current_component_status_MISSING(self, ucr, u):
    ...
    def test_get_current_component_status_OK(self, ucr, u):
        ...
        try:
            tmp = NamedTemporaryFile()
            tmp.write(b'deb http://host:port/prefix/0.0/maintained/component/ c/arch/')
            tmp.write(b'deb http://host:port/prefix/0.0/unmaintained/component/ d/arch/')
            tmp.flush()
            U.Component.FN_APTSOURCES = tmp.name
    ...
    def test__get_component_server_default(self, u):
        ...
        u.repourl.path = '/univention-repository/'
    ...
    def test_get_component_repositories_ARCH(self, ucr, http, u):
        ...
        uris = {
            'univention-repository/%d.%d/maintained/component/a/%s/Packages.gz' % (MAJOR, MINOR, 'all'): DATA,
            'univention-repository/%d.%d/maintained/component/a/%s/Packages.gz' % (MAJOR, MINOR, ARCH): DATA,
        ...
        assert {
            'deb https://updates.software-univention.de/univention-repository/%d.%d/maintained/component/ a/%s/' % (MAJOR, MINOR, 'all'),
            'deb https://updates.software-univention.de/univention-repository/%d.%d/maintained/component/ a/%s/' % (MAJOR, MINOR, ARCH),
        } == set(a_repo)
    ...
    def test_get_component_repositories_NOARCH(self, ucr, http, u):
        ...
        uris = {
            'univention-repository/%d.%d/maintained/component/a/Packages.gz' % (MAJOR, MINOR): DATA,
            RJSON: gen_releases(patches=[PATCH, PATCH + 1]),
        }
        http(uris)
        ver = U.UCS_Version((MAJOR, MINOR, PATCH))
        a_repo = u.component('a').repositories(ver, ver)
        assert {
            'deb https://updates.software-univention.de/univention-repository/%d.%d/maintained/component/a/ ./' % (MAJOR, MINOR),
        } == set(a_repo)
    ...
    def test_print_component_repositories(self, ucr, http, u):
        ...
        uris = {
            'univention-repository/%d.%d/maintained/component/%s/%s/Packages.gz' % (MAJOR, MINOR, 'a', 'all'): DATA,
            'univention-repository/%d.%d/maintained/component/%s/%s/Packages.gz' % (MAJOR, MINOR, 'a', ARCH): DATA,
            RJSON: gen_releases(patches=[PATCH, PATCH + 1]),
        }
        http(uris)
        tmp = u.print_component_repositories()
        assert {
            'deb https://updates.software-univention.de/univention-repository/%d.%d/maintained/component/ %s/%s/' % (MAJOR, MINOR, 'a', 'all'),
            'deb https://updates.software-univention.de/univention-repository/%d.%d/maintained/component/ %s/%s/' % (MAJOR, MINOR, 'a', ARCH),
        } == set(tmp.splitlines())
