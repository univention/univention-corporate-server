...
log = Structured(logging.getLogger('LICENSE'))
...
class _KeyStore:
    ...
    def verify(self, data: bytes, b64_signature: str) -> bool:
        ...
        try:
            ...
            log.error('Invalid base64: %r', b64_signature)
            return False
...
class _License:
    ...
    def __init__(self) -> None:
        ...
        self._lo: univention.uldap.access | None = None

    @property
    def lo(self) -> univention.uldap.access:
        if self._lo is None:
    ...
    def _get_search_paths(self) -> list[str]:
        ...
        paths = self.lo.get(directory_dn, ['univentionLicenseObject']).get('univentionLicenseObject', [])
        if not paths:
    ...
    def _check_basedn(self, data: LicenseData, base_dn: str) -> bool:
        ...
        if univention.uldap.access.compare_dn(license_base.lower(), base_dn.lower()):
            return True
