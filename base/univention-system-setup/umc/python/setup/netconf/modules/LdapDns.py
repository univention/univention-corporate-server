...
class PhaseLdapDns(AddressMap, Ldap, Executable):
    ...
    def _create_reverse_dns_ipv4(self) -> None:
        for ipv4 in set(self.changeset.new_ipv4s) - set(self.changeset.old_ipv4s):
            with tempfile.NamedTemporaryFile(mode="w") as bindpwd_file:
                bindpwd_file.write(self.bindpwd)
                bindpwd_file.flush()
                self.call([self.executable, "--binddn", self.binddn, "--bindpwdfile", bindpwd_file.name, "--ignore-exists", "--reverse", ipcalc.calculate_ipv4_reverse(ipv4), "add", "zone", *self._soa()])
                self.call([
                    self.executable,
                    "--binddn", self.binddn,
                    "--bindpwdfile", bindpwd_file.name,
                    "--ignore-exists",
    ...
    def _create_reverse_dns_ipv6(self) -> None:
        for ipv6 in set(self.changeset.new_ipv6s) - set(self.changeset.old_ipv6s):
            with tempfile.NamedTemporaryFile(mode="w") as bindpwd_file:
                bindpwd_file.write(self.bindpwd)
                bindpwd_file.flush()
                self.call([self.executable, "--binddn", self.binddn, "--bindpwdfile", bindpwd_file.name, "--ignore-exists", "--reverse", ipcalc.calculate_ipv6_reverse(ipv6), "add", "zone", *self._soa()])
                self.call([
                    self.executable,
                    "--binddn", self.binddn,
                    "--bindpwdfile", bindpwd_file.name,
