			...
			if not device.ip4dynamic:
				for ip4 in device.ip4:
					...
					try:
						...
						raise ValueError('%s: invalidn IPv4 address %r' % (device.name, ip4[0])
					all_ip4s.add(ip4[0])
			if not device.ip6dynamic:
				for ip6 in device.ip6:
					...
					try:
						...
						raise ValueError('%s: invalidn IPv6 address %r' % (device.name, ip6[0])
					all_ip6s.add(ip6[0])
...
class Device(object):
	...
	def parse_ucr(self):
		...
		for key in vals.copy():
			...
			if key in vals:
				self.leftover.append((key, vals.pop(key)))
