...
#ROOT Device
root_device=`python2.4 -OOO /sbin/univention-baseconfig get installer/device/0/name`
root_fs=`python2.4 -OOO /sbin/univention-baseconfig get installer/device/0/fs`
if [ "$root_fs" = "xfs" ]; then
	options="defaults"
else
...
eval `python2.4 -OOO /sbin/univention-baseconfig shell`

set | egrep -v "installer_device_0_mp=/" | egrep "installer_device_.*name=" | while read line; do
	device_number=`echo $line | awk -F _ '{print $3}'`
	device=`python2.4 -OOO /sbin/univention-baseconfig get installer/device/$device_number/name`
	mp=`python2.4 -OOO /sbin/univention-baseconfig get installer/device/$device_number/mp`
	fs=`python2.4 -OOO /sbin/univention-baseconfig get installer/device/$device_number/fs`
