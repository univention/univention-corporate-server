...
FOR /F "delims=. tokens=1" %%I IN ('DIR /B "%SYSTEMROOT%\INF\OEM*.INF"') DO (
 TYPE "%SYSTEMROOT%\INF\%%I.inf" | FIND /c /i "xen"  >james
 FOR /f %%c IN (james) DO (
  IF /I %%c NEQ 0 (
...
REM clean registry
REG DELETE HKLM\SYSTEM\CurrentControlSet\Services\XenConfig /f
...
REG DELETE HKLM\SYSTEM\CurrentControlSet\Services\XenStub /f
