...
* During the |UCS| domain join process, slapd fails to start if schema lines
  exceed 2000 bytes. The issue is resolved by wrapping long attributeType and
  objectClass lines at 1500 characters to prevent this failure
  (:uv:bug:`56247`).
...
* Further improvments for the delegative administration feature have been
  implemented (:uv:bug:`58517`).

* Clean up references to apps installed on a Domaincontroller or memberserver
  when the computer is removed (:uv:bug:`54892`).
...
* Fixed the directory where the appcenter stores the cache for the next release
  during ``univention-app update-check`` (:uv:bug:`58240`).
...
* Disable an unnecessary Systemd service when installing the Self Service App
...
* This update ships a new UMC diagnostic plugin ``71_samba_memberOf`` which
  checks that the ``memberOf`` attribute is visibible in the output of
  ``univention-s4search`` and offers possible measures in case that is not the
  case. This errata update is a followup to the knowledge base article
  https://help.univention.com/t/18673 (:uv:bug:`53882`).
...
* The default global search container (i.e., "All containers") can now be
  disabled via the |UCSUCRV| :envvar:`directory/manager/web/modules/search/global-
  search`. When disabled, the |UCSUCRV| :envvar:`directory/manager/web/modules/
  search/default-search` can be enabled to limit
  searches to module-specific default containers. This improves search
...
* The script ``univention-update-univention-object-identifier`` now provides a
  ``--dry-run`` option (:uv:bug:`58446`).

* ``ldap_setup_index`` now checks if number of indexed attributes would exceed
  maximum number of lmdb sub-databases (:uv:bug:`58443`).
...
* The pre-update script has been updated to run ``univention-prune-kernels``, in
...
* The JFrog Artifactory with authentication return a 403 instead of 401 when
  authentication is missing, this is not correct, to solve that problem a
  preemptive authentication was added which first tries it with credentials,
  but then also proceeds if they are url-encoded (:uv:bug:`58371`).
...
* Add a ``--force` flag to ``oidc/rp create`` and ``saml/sp create`` which updates
  existing Keycloak clients to the configuration given by the command
  (:uv:bug:`58426`).

* The command ``univention-keycloak saml-client-nameid-mapper create`` was not
  idempotent and failed with a traceback if the mapper already existed, making
  it unsuitable for the use in joinscripts. This has been fixed
  (:uv:bug:`58544`).
...
* The logfile ``/var/log/univention/radius_ntlm_auth.log`` is no longer emptied
  during package updates (:uv:bug:`58425`).

* In some situations, ``univention-radius-ntlm-auth`` did not correctly report
  errors to the :program:`RADIUS` server and also did not log them. The program has been
  improved and is now able to intercept these errors and log them in
  ``/var/log/univention/radius_ntlm_auth.log`` (:uv:bug:`58132`).

* The permissions for the log file ``/var/log/univention/radius_ntlm_auth.log``
  were not set correctly by logrotate, which caused ``univention-radius-ntlm-
  auth`` to crash. This update automatically corrects the file permissions and
  the configuration of logrotate (:uv:bug:`58132`).
...
* Pre-create the AD builtin groups ``Pre-Windows 2000 Compatible Access``,
  ``Windows Authorization Access Group`` and ``IIS_IUSRS`` via |UDM| in the
  joinscript. This is required because we put those groups on the
  ``connector/s4/mapping/group/ignorelist``, but we want them to be defined with
  static POSIX IDs across the |UCS| domain. As a result these groups are created
  with the ``hidden`` flag now, so they don't show up in UMC for new |UCS| domains.
  That is okay, because they are not to be administrated in any way, they just
  allocate a POSIX ID. This errata update is a followup to the knowledge base
  article https://help.univention.com/t/18673 (:uv:bug:`53882`).

* Samba 4.21 had a regression where ``samba-tool domain trust create`` failed to
  create the trust object. The upstream patch for Samba 4.22 has been
  backported to fix this (:uv:bug:`58299`).
...
* Add the AD builtin groups ``Pre-Windows 2000 Compatible Access``, ``Windows
  ...
  (e.g. for ``univention-s4search``). By default it contains the virtual group
  ``Authenticated Users``, but may be configured differently in Samba/AD for
  security reasons. This errata update is a followup to the knowledge base
  article https://help.univention.com/t/18673 (:uv:bug:`53882`).
...
* Introduced a new |UCSUCRV| :envvar:`connector/ad/mapping/allow-subtree-
  ancestors`, which, if enabled, allows the synchronization of ancestors of
  subtrees allowed with the |UCSUCRVs| :envvar:`connector/ad/mapping/allowsubtree
  /.*/[ad|ucs]`. This can make the management
  of the selective synchronization of more complex LDAP DIT structures simpler.
  Additionally, when this new variable is enabled, a resynchronization with one
  of the ``resync_object_from_*`` scripts will handle the resynchronization of
  ancestors automatically if necessary (:uv:bug:`57979`).

* Slight adjustments for the experimental delegative administration feature
  have been done (:uv:bug:`58432`).

* In cases where customers chose LDAPS to bind to Active Directory (by setting
  ``connector/ad/ldap/port=636`` and ``connector/ad/ldap/ldaps=yes`` in UCR) the
  script ``univention-adsearch`` aborted with ``NT_STATUS_INVALID_PARAMETER_MIX``.
  Now it passes the parameters ``tls cafile`` and ``tls crlfile`` to ``ldbsearch`` to
  avoid that error message (:uv:bug:`56139`).
