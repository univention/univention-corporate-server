...
\item When slapd could not be started by its initscript, slapschema is called to determine the cause and the output is displayed (\ucsBug{23055}).
...
\item The following syntax classes received lookup optimizations
  ...
  ,,DomainController'' (\ucsBug{30991}, \ucsBug{31560}).
...
\item Add dependencies of \ucsName{univention-printserver} and \ucsName{bsd-mailx} to \ucsName{univention-check-printers}  (\ucsBug{29123}).
...
\subsection{Samba 4}
\begin{itemize}
\item \ucsUCRV{windows/wins-server} and \ucsUCRV{windows/wins-support} are now initiated when joining  (\ucsBug{30813}).
...
\item \ucsName{univention-debmirror}. When uninstall package symlink
'/etc/apache2/sites-enabled/univention-repository' will be removed.
(\ucsBug{29262}).

\item The package \ucsName{univention-local-users} is not longer maintained
(\ucsBug{29835}).
