...
\begin{itemize}
\item The LDAP schema is now checked in preup.sh if slapd is installed (\ucsBug{25674}).
\end{itemize}
...
\item Samba package lists have been updated to fix deinstallation issues (\ucsBug{30982}).
...
\item The Linux kernel was updated to 3.10-rc7. The nouveau KMS kernel
  ...
  The firmware packages have been updated to current versions as well.
...
\item The \ucsUCRV{ldap/server/port} is now set to 7389 by default (\ucsBug{26888)}.

\item The LDAP attribute \ucsName{univentionShareHost} now supports the
substring search (\ucsBug{31208}).
...
\item The failed LDIF mode of the replication does now handle empty values
correctly (\ucsBug{25579}).
...
\item Fixed problems in the login dialog when changing an expired password.
  Under some circumstances the username sent was wrong and the standby
  animation did never stop (\ucsBug{31898}).
...
\item In \ucsName{umc/widgets/Grid:actions} a specific tooltip class can now be defined for actions (\ucsBug{31308}).
...
\item If an application is installed, the system does not dist-upgrade anymore.
  ...
  \ucsBug{31278}).
\item Removed HTML evaluation of localised values in ini file (\ucsBug{31185}).
...
\item Checking hardware requirements when upgrading an app has been disabled
  (\ucsBug{31354}).
\item Added helpful messages when new license is not yet installed and no UDM
  is present (\ucsBug{30629}).
\item Supporting new HTML READMEs INSTALL, POST\_UPDATE, POST\_INSTALL (\ucsBug{31803}).
\item Keeping session alive after having installed an app until confirmation (\ucsBug{31886}).
\item Speedup updates by committing repository files only once (\ucsBug{31228}).
...
\item Opening superordinates which were recently created did not opened correctly (\ucsBug{25008}).
\end{itemize}
...
\begin{itemize}
\item The recommendation for setting up a UCR policy has been fixed to mention the correct \ucsUCRV{updater/secure\_apt} (\ucsBug{24808}).
\item A locally required and mirrored component from a separate repository in same cases blocked the update of the local repository. This has been changed to now always update the local repository as soon as a new release is available. The condition is still check by each host individually (\ucsBug{31060}).
\item The pre- and post-updater scripts of locally mirrored but disabled components are now also retrieved (\ucsBug{30756}).
...
\item The function \ucsName{getLDAPURIs} has been added to the python lib. This
function returns a string with all configured LDAP URIs (\ucsBug{19753}).

\item The function \ucsName{getLDAPServersCommaList} has been added to the
python lib. This function returns a comma-seperated string with all configured
LDAP servers (\ucsBug{19753}).

\item Added UMCConnection to the python lib, moved from the \ucsName{Univention
  App Center} (\ucsBug{31058}, \ucsBug{31908}).
...
\item The cyrus configuration now uses the LDAP servers configured through the
\ucsUCRV{ldpa/server/addition} too (\ucsBug{19753}).

\item The PAM tool \ucsName{pam\_univentionmailcyrus} supports a comma-seperated
list for the option \ucsName{ldap\_host} (\ucsBug{19753}).
\end{itemize}
...
\item The postfix configuration now uses the LDAP servers configured through the
\ucsUCRV{ldpa/server/addition} too (\ucsBug{19753}).
...
\item Modified in \ucsName{univention-printserver} univention-ldapsearch in join scripts to request LDAP on DC-Master not on local DC. Credentials of the join user will be used for authentication.(\ucsBug{19101}).
...
\item The process \ucsName{kpasswdd} is now only started only on the UCS master (ucsBug{20167}).
\end{itemize}
...
\item By default the mysql server package does not set a password for the mysql root user during a
noninteractive installation. With this update the mysql package sets a password and writes the
password to /etc/mysql.secret. This file can be read by root.  If a password is already set, nothing
will be done (\ucsBug{19354}).
...
\item Creating snapshots of suspended virtual machines is prevented (\ucsBug{23436}).
\item The lables to enable VNC have been synchronized (\ucsBug{31946}).
\end{itemize}
...
\item To minimize the time where \ucsURL{/etc/samba/shares.conf} includes non-existing files, it is now updated directly after a share was added/removed or renamed (\ucsBug{29399}).
\item The \ucsUCRV{samba/max\_protocol} has been renamed to \ucsUCRV{samba/max/protocol} (\ucsBug{31921}).
\end{itemize}
...
\item Support for the \ucsUCRV{samba/max/protocol} has been added (\ucsBug{31145}).
\item Samba4 DCs are now configured to use the local KDC (\ucsBug{29291}).
\item The config file \ucsURL{/etc/krb5.conf} will get commited from the updated UCR template on update (\ucsBug{31306}).
\item The default for the \ucsUCRV{samba4/sysvol/sync/setfacl/AU} has been changed to \emph{false}, so the sysvol-sync script does not set read access fACLs for the group \emph{Authenticated Users} each time it is run by cron (\ucsBug{31275}).
\end{itemize}
...
\item Logging of add and modify operations against the Samba 4 directory has been improved, especially in the case of a traceback (\ucsBug{31133}).
\end{itemize}
...
The robustness against DRS and rsync replication delays has been improved in some tests (\ucsBug{28343}, \ucsBug{28348}, \ucsBug{29057}, \ucsBug{29058}, \ucsBug{29062}).

\item The Samba3 join test has been removed (\ucsBug{31835}).

\item Samba4 NTACL tests have been improved a bit (\ucsBug{29230}). 

\item The tests for Samba share permissions have been improved (\ucsBug{29054}).
...
\item \ucsName{univention-user-quota} has been rewritten in python. It does
  now support share names with whitespaces (\ucsBug{30636}, \ucsBug{31957}).
