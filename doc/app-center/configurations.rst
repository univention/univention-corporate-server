...
user enters in the dialog takes precedence. Stored values, including existing
file contents and container settings, are preserved when an upgrade introduces
a setting for them. During an upgrade, a new setting receives the initial
value only if a fresh installation also sets it. For
