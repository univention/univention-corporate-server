...
user enters in the dialog takes precedence. If a value already exists, for
example in a file or as a container variable, the upgrade keeps it. During an
upgrade, a setting receives the initial value only if a fresh installation
also sets it. For Docker apps, this applies to every setting that the upgrade
introduces. For apps without Docker, it applies to settings that list
``Install`` in ``Show``. The initial value isn't a default. If you remove the
value later, for example with :command:`univention-app configure --unset`,
the setting stays empty.

The attribute ``Required`` defines whether the setting must have a value. You
can't unset a required setting or set it to an empty value. Use it for
settings that the app can't work without.
