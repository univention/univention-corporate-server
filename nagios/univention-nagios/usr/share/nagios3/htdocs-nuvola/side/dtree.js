...
|                                                   |
| Updated: 17.04.2003                               |
| Updated: 25.05.2005 gael.pourriel@gmail.com       |
| -Added XP Style headers                           |
|--------------------------------------------------*/

function changeMenuState(menuID)
{
	var menuDiv = "menu_div_" + menuID;
	var menuImg = "menu_img_" + menuID;
	...
	if ( currentState == "Collapse" )
	{
		...
		if (document.images)
			document.getElementById(menuImg).src = "side/expand.gif";
	...
	else
	{
		...
		if (document.images)
			document.getElementById(menuImg).src = "side/collapse.gif";
	}
}
...
function dTree(objName) {
	...
	this.icon = {
		root				: 'side/icons/home.gif',
		folder				: 'side/empty.gif',
		folderOpen			: 'side/empty.gif',
		node				: 'side/empty.gif',
		empty				: 'side/empty.gif',
		line				: 'side/empty.gif',
		join				: 'side/empty.gif',
		joinBottom			: 'side/empty.gif',
		plus				: 'side/empty.gif',
		plusBottom			: 'side/empty.gif',
		minus				: 'side/minus.gif',
		minusBottom			: 'side/empty.gif',
		nlPlus				: 'side/plus.gif',
		nlMinus				: 'side/minus.gif'
	};
	this.obj = objName;
	this.aNodes = [];
	this.aIndent = [];
...
var header = '';
var footer = '</td></tr></tbody></table></div></td></tr></tbody></table>';
...
dTree.prototype.header = function(title,icon,height,bgImage,bgColor,open) {
	...
	header += '<tr>';
	header += '<td>';
	header += '<div width="100%" style="';
	
	if ( open )
		header += 'height: 100%;overflow: visible;" title="Collapse"';
	else
		header += 'height: ' + height +'px;overflow: hidden;" title="Expand"';
	
	header += ' id="menu_div_' + this.obj + '">';
	
	header += '<table border="0" cellpadding="0" cellspacing="0" width="100%">';
	header += '<tbody>';
	header += '<tr style="cursor: pointer;" onclick="changeMenuState(\'' + this.obj + '\')">';
	header += '<td height="1" width="100%">';
	header += '<table border="0" cellpadding="0" cellspacing="0" width="100%">';
	header += '<tbody>';
	header += '<tr>';
	header += '<td><img src="' + icon + '"></td>';
	header += '<td height="100%" width="100%">';
	header += '<table border="0" cellpadding="0" cellspacing="0" height="100%" width="100%">';
	header += '<tbody>';
	header += '<tr>';
	header += '<td colspan="2"><img src="side/blank.gif" height="9" width="0"></td>';
	header += '</tr>';
	header += '<tr>';
	...
	if ( open )
		header += 'side/collapse.gif';
	else
		header += 'side/expand.gif';
...
dTree.prototype.node = function(node, nodeId) {
	...
	var str = '<div class="' + node.css +'">' + this.indent(node, nodeId);
	if ( this.root.id != node.pid || this.config.showRoot == true ) {
