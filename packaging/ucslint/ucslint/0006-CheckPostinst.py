...
class UniventionPackageCheck(uub.UniventionPackageCheckDebian):
	...
	def getMsgIds(self):
		return {
			...
			'0006-9': [ uub.RESULT_WARN, 'script contains UCR variable server/role or $server_role - this may be undefined during package installation!'],
		}
		...
		for fn, checks in fnlist_scripts.items():
			...
			for linecnt, line in enumerate(lines):
				...
				if fn.endswith('postinst') or fn.endswith('preinst'):
					if UniventionPackageCheck.RE_SERVER_ROLE.search(line):
						self.addmsg('0006-9', 'script contains UCR variable server/role or $server_role - this variable may be undefined during package installation!', fn, linecnt+1)
