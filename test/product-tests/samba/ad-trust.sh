...
check_trust () {
	...
	# dns forward in UCS
	local win_hostname="$(ucs-winrm run-ps  --cmd '$env:computername' --loglevel error | head -1 | tr -d '\r')"
...
%EOR
	...
	! ucs-winrm domain-user-validate-password --domain "$WINRM_DOMAIN" --domainpassword wrong_password --domainuser ucs1
	! ucs-winrm domain-user-validate-password --domain INVALID_DOMAIN --domainpassword univention --domainuser ucs1
