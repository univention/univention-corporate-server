#!/bin/bash
# SPDX-FileCopyrightText: 2018-2023 Univention GmbH
...
*) exec ssh -o BatchMode=yes "${KVM_BUILD_SERVER:?}" "KVM_BUILD_SERVER='${KVM_BUILD_SERVER:?}' APP_ID='${APP_ID:?}' UCS_VERSION='${UCS_VERSION:?}' bash -s" <"$0" ;;
esac
