...
sed -i "s/8\.32\.192/8\.33\.241/g" install.sh values.*
...
cat <<'EOF' >values.secret.yaml.patch
@@ -20,6 +20,7 @@
     auth:
       rootPassword: "HCYXHcE4/_5Jl.pU"
   secretProperties:
+    com.openexchange.dovecot.doveadm.apiSecret: "WmuJvUUr.56FEBC9"
     com.openexchange.filestore.s3client.s3.accessKey: "a23105750442f3e8226df3e817b2e9f1"
     com.openexchange.filestore.s3client.s3.secretKey: "41aab9492c60ed4af5f5a9c6c99e944b"
     com.openexchange.mail.masterPassword: "Iikez~kT3hOihFWh"
EOF
patch values.secret.yaml <values.secret.yaml.patch

#cd ../../dovecot-ce/helm/dovecot-ce/templates/statefulset.yaml
cat <<'EOF' >statefulset.yaml.patch
@@ -60,9 +60,6 @@
             - name: sieve
               containerPort: {{ .Values.service.sieve.port }}
               protocol: TCP
+            - name: doveadm
+              containerPort: {{ .Values.service.doveadm.port }}
+              protocol: TCP
           {{- if .Values.doveconf.tls.enabled }}
           env:
             - name: TLS_KEY
EOF
patch ../../dovecot-ce/helm/dovecot-ce/templates/statefulset.yaml <statefulset.yaml.patch

cat <<'EOF' >batteries.values.dovecot-ce.patch
@@ -57,6 +57,8 @@
   sieve:
     port: 4190
     nodePort: 30419
...
+    port: 8080
 ...
 ingress:
   enabled: false
EOF
patch ../../dovecot-ce/helm/dovecot-ce/values.yaml <batteries.values.dovecot-ce.patch
