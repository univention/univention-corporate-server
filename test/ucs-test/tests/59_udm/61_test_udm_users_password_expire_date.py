...
def ldap_obj_search(lo, dn=None, ldapfilter='(objectClass=*)'):
    if dn:
...
def modify_and_diff(udm, lo, **kwargs):
    before = ldap_obj_search(lo, kwargs['dn'])
    udm.modify_object('users/user', **kwargs)
    utils.wait_for_connector_replication()
    after = ldap_obj_search(lo, kwargs['dn'])
    return dictdiff(before, after)


def modify_and_check_expectation(udm, lo, expected_diff=None, **kwargs):
    if expected_diff:
        diff = modify_and_diff(udm, lo, **kwargs)
...
def test_udm_users_password_expire_date(udm, lo):
    ...
    expected_after = ldap_obj_search(lo, userdn)
    ...
    modify_and_check_expectation(udm, lo, expected_diff, dn=userdn, userexpiry=new_uexp)
    ...
    expected_diff = dictdiff(expected_before, expected_after)
    modify_and_check_expectation(udm, lo, expected_diff, dn=userdn, userexpiry=new_uexp, disabled="0")
    ...
    expected_diff = dictdiff(expected_before, expected_after)
    modify_and_check_expectation(udm, lo, expected_diff, dn=userdn, userexpiry=new_uexp, disabled="1")
    ...
    # Modify and check
    expected_diff = {}
    modify_and_check_expectation(udm, lo, expected_diff, dn=userdn, disabled="0")

    # Modify and check
    modify_and_check_expectation(udm, lo, expected_diff, dn=userdn, userexpiry="")
