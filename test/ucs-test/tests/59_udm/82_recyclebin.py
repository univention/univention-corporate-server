...
@pytest.mark.skip(reason="Policy detection _should_recycle_object() method not yet implemented in handlers")
def test_policy_detection_working(recyclebin_policy_session, udm, lo):
    ...
    user_obj = user_module.object(None, lo, pos, dn=user_dn)
    user_obj.open()
    assert user_obj._should_recycle_object()
...
def test_entryuuid_univention_object_identifier_preservation_across_delete_restore(udm, lo, recyclebin_policy_session):
    ...
    udm.remove_object('users/user', dn=user_dn)
    deleted_obj_dn = _deleted_object_dn(user_dn)
...
def test_listener_cache_behavior(udm, lo, recyclebin_policy_session):
    ...
    deleted_dn1 = _deleted_object_dn(user_dn1)
    deleted_dn2 = _deleted_object_dn(user_dn2)
...
def test_recyclebin_type_limitation(udm, lo, recyclebin_policy_session):
    ...
    deleted_dn = _deleted_object_dn(computer_dn)
    verify_ldap_object(deleted_dn, should_exist=False)
...
def test_uuid_lookup_from_recyclebin_when_group_deleted_first(udm, lo, recyclebin_policy_session):
    ...
    import subprocess
    subprocess.run([
        '/usr/sbin/udm-test', 'groups/group', 'remove',
        '--dn', group_dn,
    ], check=True)
    verify_ldap_object(group_dn, should_exist=False)
    deleted_group_dn = _deleted_object_dn(group_dn)
    ...
    for ref in stored_references:
        parsed = Reference.parse(ref)
        if parsed and parsed.target_module == 'groups/group' and parsed.lookup_value == group_uuid:
    ...
    assert test_group_found, f"Should have found a reference with test group UUID {group_uuid}. References: {stored_references}"
