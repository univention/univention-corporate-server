...
def test_nonzero(fs_type: str) -> None:
    "Checking if setting of Quota works with two shares on a mountpoint, with none of the values being zero."
...
@pytest.mark.parametrize("fs_type", ["ext4", "xfs"])
def test_zero(fs_type: str) -> None:
    "Checking if setting of Quota works with two shares on a mountpoint, with some of the values being zero."
    quotaCheck = QuotaCheck(quota_type="usrquota", fs_type=fs_type)
    quota_policies = [{
        'name': 'quota_policy1',
        'spaceSoftLimit': '0',
...
@pytest.mark.parametrize("fs_type", ["ext4", "xfs"])
def test_one_policy(fs_type: str) -> None:
    "Checking if setting of Quota works with two shares on a mountpoint, with only one of them having a quota policy attached."
    quotaCheck = QuotaCheck(quota_type="usrquota", fs_type=fs_type)
