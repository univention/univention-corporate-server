...
COMMON_EXCEPTIONS = (
    ...
    E("univention.udm.exceptions.NoObject: No object found at DN 'cn=internal-name-for-(folder|category|entry)", ['in refresh'], 53333),  # 86_browser/185_portal_administration_inline_creation  # Bug #53333
    E("univention.admin.uexceptions.noObject:.*cn=internal-name-for-(folder|category|entry),cn=(entry|category),cn=portals", None, 53333),  # 86_browser/185_portal_administration_inline_creation  # Bug #53333
    ...
    E(r"AttributeError: 'Logger' object has no attribute '_fallbackLogger'", ['_reverse_umc_module_logger']),
    E(r"ImportError: cannot import name 'pki_option' from 'univention.admin.certificate' (/usr/lib/python3/dist-packages/univention/admin/certificate.py)"),
)
