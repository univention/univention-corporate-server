# Path: test/ucs-test/tests/91_udm-performance-test/locustfiles/tasks//home/ivan/univention/ucs/test/ucs-test/tests/91_udm-performance-test/locustfiles/tasks/computers_domaincontroller_slave/computers_domaincontroller_slave_get.py
...
# # - GET /computers/domaincontroller_slave/
#


def computers_domaincontroller_slave_get(self):
    """GET /computers/domaincontroller_slave/"""
    # query parameters for this endpoint
    query = {
        'filter': 'name=*',  # A LDAP filter which may contain `UDM` property names instead of `LDAP` attribute names. (string)
    ...
    self.request('get', '/univention/udm/computers/domaincontroller_slave/', name='/computers/domaincontroller_slave/', headers=header, params=query, verify=False, response_codes=[200])
