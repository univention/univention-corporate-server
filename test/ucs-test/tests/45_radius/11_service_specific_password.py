...
def test_udm_rest(ucr, udm_session, rad_user):
    ...
    ucr.handler_set(['radius/use-service-specific-password=true'])
...
def test_radius_auth_without_ssp(ucr, udm_session, rad_user):
    _dn, name, password = rad_user
    ucr.handler_set(['radius/use-service-specific-password=false'])
    radius_auth(name, password)


# auth should fail with either password, there is no fallback
def test_radius_auth_no_ssp(ucr, rad_user, ssp):
    _dn, name, password = rad_user
    ucr.handler_set(['radius/use-service-specific-password=true'])
    with pytest.raises(subprocess.CalledProcessError):
...
def test_radius_auth_ssp(ucr, rad_user, lo, ssp):
