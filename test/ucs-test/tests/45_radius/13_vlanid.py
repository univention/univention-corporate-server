...
def test_user_vlan_id(udm_session, vlan_id_group_one, vlan_id_group_two, ucr_vlan_id, expected_vlan_id, restart_freeradius, subtests: SubTests):
    ...
    for auth_method in ('pap', 'mschap', 'eap', 'peap'):
        with subtests.test('radius auth method', auth_method=auth_method):
            assert radius_auth(username, 'univention', 'user',
                               auth_method) == expected_vlan_id, f'wrong vlan-id for auth method {auth_method!r}'
