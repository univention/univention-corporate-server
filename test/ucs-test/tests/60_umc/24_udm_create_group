...
class TestUMCGroupsCreation(UMCBase):
    ...
    def add_group_policy(self, groupname, policyname):
        ...
        try:
            ...
            if not request_result:
                utils.fail("Request 'udm/put' to add policy to a group with options '%s' failed, hostname %s" % (options, self.hostname))
            if not request_result[0].get('success'):
                utils.fail("Request 'udm/put' to add policy to a group with options '%s' failed, no success = True in response, hostname %s" % (options, self.hostname))
        except Exception as exc:
    ...
    def main(self):
        ...
        try:
            ...
            print("Adding a '%s' policy to a test group '%s'" % (policyname, test_groupname))
            self.add_group_policy(test_groupname, policyname)

            print("Checking test group '%s' policies for '%s'" % (test_groupname, policyname))
            group_policies = self.get_group_by_name(test_groupname)[0].get('$policies$')
            if not group_policies:
                utils.fail("The group policies for '%s' are empty or not present after '%s' policy was applied to group" % (test_groupname, policyname))

            group_policies = group_policies.get('policies/umc')
            if not group_policies:
                utils.fail("The group 'policies/umc' field for '%s' is empty or not present after '%s' policy was applied to group" % (test_groupname, policyname))
            if not ('cn=' + policyname + ',cn=UMC,cn=policies,' + self.ldap_base in group_policies):
                utils.fail("The policy '%s' is not in a group '%s' policies '%s'" % (policyname, test_groupname, group_policies))
        finally:
