...
class TestUMCUserCreation(UDMModule):
    ...
    def modify_user_groups(self, username, groupnames):
        ...
        if not request_result:
            utils.fail("Request 'udm/put' to change user '%s' groups with options '%s' failed, hostname %s" % (username, options, self.hostname))
        if not request_result[0].get('success'):
            utils.fail("Request 'udm/put' to change user '%s' groups with options '%s' failed, no success = True in response, hostname %s" % (username, options, self.hostname))
