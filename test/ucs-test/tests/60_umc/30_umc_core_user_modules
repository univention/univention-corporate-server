...
class TestUMCUserModules(UMCBase):
	...
	def main(self):
		...
		with udm_test.UCSTestUDM() as udm:
			...
			assert len(user_modules) == 0, "The newly created test user '%s' in test group '%s' has access to the following modules '%s', when should not have access to any" % (test_username, test_groupname, user_modules)
