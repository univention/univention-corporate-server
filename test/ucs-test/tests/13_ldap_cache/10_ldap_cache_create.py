...
def test_cache_group_rename(udm, group1, user1):
	...
	assert group1 not in data_uids
	assert group1 not in data_members
	assert data_uids[new_group_dn] == [explode_dn(user1, 1)[0]]
	assert data_members[new_group_dn] == [user1]
...
def test_cache_user_rename(udm, group1, user1):
	...
	# new user in cache
	assert data_uids[group1] == [explode_dn(new_user_dn, 1)[0]]
	assert data_members[group1] == [new_user_dn]
