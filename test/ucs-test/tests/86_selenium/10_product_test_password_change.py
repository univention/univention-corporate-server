...
from univention.testing.ucs_samba import password_policy
...
class UMCTester(object):
	...
	def test_umc(self):
		...
		self.check_for_password_reuse_error(self.admin)
	...
	def set_change_password_on_login_flag(self, user):
		...
		self.selenium.wait_for_text(_('Change password on next login'))
		self.selenium.click_text(_('Change password on next login'))
		users.save_details()
