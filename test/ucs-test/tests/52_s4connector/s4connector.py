...
class S4Connection(ldap_glue.ADConnection):
    ...
    def __init__(self, configbase: str = 'connector') -> None:
...
def check_object(object_dn: str, sid=None, old_object_dn=None) -> None:
    S4 = S4Connection()
...
def _replace_uid_with_cn(dn: str) -> str:
...
def verify_users(group_dn, users) -> None:
...
def modify_username(user_dn: str, new_user_name: str, udm_instance) -> str:
...
def connector_running_on_this_host() -> bool:
    return configRegistry.is_true("connector/s4/autostart")


def exit_if_connector_not_running() -> None:
...
def wait_for_sync(min_wait_time: int = 0) -> None:
...
def connector_setup(sync_mode: str):
