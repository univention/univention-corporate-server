...
def test_linux_client_search_scope(ou, ldap_base, linux_client_manager_user, test_computer_in_ou, test_computer_at_base, udm):
    ...
    res_base = client.umc_command('udm/query', options_base, 'computers/linux').result
    found_dns_base = {x['$dn$'] for x in res_base}

    # Should not find computers outside their OU
    assert test_computer_at_base not in found_dns_base
...
@check_delegation
def test_linux_client_manager_cannot_modify_group_memberships_in_ou(ou, ldap_base, linux_client_manager_user, test_computer_in_ou, test_group_in_ou):
    """Test that linux client manager cannot modify group memberships for computers even within their OU."""
    client = Client()
    client.authenticate(ou.client_manager_username, 'univention')
...
@check_delegation
def test_linux_client_manager_cannot_modify_group_memberships_cross_ou(ou, ldap_base, linux_client_manager_user, test_computer_in_ou, test_group_at_base):
    """Test that linux client manager cannot modify group memberships for groups outside their OU."""
    client = Client()
    client.authenticate(ou.client_manager_username, 'univention')

    changes = {
        '$dn$': test_group_at_base,
        'hosts': [test_computer_in_ou],
    }

    res = client.umc_command('udm/put', [{'object': changes}], 'groups/group').result[0]
    assert res['success']
    assert res['$dn$'] == test_group_at_base


@check_delegation
def test_linux_client_manager_can_modify_group_hosts_at_base_cross_ou(ou, ldap_base, linux_client_manager_user, test_computer_in_ou, test_group_at_base):
    """Test that linux client manager can modify group hosts for base-level groups."""
    client = Client()
    client.authenticate(ou.client_manager_username, 'univention')
