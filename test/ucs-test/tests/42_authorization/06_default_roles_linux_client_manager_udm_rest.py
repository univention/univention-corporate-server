...
from univention.admin.rest.client import UDM as UDM_REST, Forbidden, NotFound
...
@check_delegation
def test_linux_client_manager_cannot_modify_group_memberships_in_ou(ou, test_computer_in_ou, test_group_in_ou):
    """Test that linux client manager cannot modify group memberships for computers via UDM-REST."""
    udm_rest = create_udm_rest_client(ou.client_manager_username)
...
@check_delegation
def test_linux_client_manager_can_modify_group_hosts_at_base(ou, test_computer_in_ou, test_group_at_base):
    """Test that linux client manager can modify group hosts for base-level groups via UDM-REST."""
    udm_rest = create_udm_rest_client(ou.client_manager_username)
    groups_module = udm_rest.get('groups/group')

    group = groups_module.get(test_group_at_base)
    current_hosts = group.properties.get('hosts', [])
    group.properties['hosts'] = [*current_hosts, test_computer_in_ou]
    group.save()
