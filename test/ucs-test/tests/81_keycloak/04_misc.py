...
def test_session_sync(ucr, udm, portal_login_via_keycloak, portal_config, keycloak_config, protocol):
    ...
    addresses = [addr.address for addr in dns.resolver.resolve(fqdn, 'A')]
...
def test_csp(keycloak_config, ucr, is_keycloak):
    ...
    assert f"*.{ucr['domainname'].lower()}" in response.headers['Content-Security-Policy'].lower()
