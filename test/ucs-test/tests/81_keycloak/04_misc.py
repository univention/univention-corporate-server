...
def test_session_sync(ucr, udm, portal_login_via_keycloak, portal_config, keycloak_config):
    ...
    login_host = ldap.search(f"(&(arecord={login_ip})(objectClass=univentionHost))", attr=[
                             "cn"])[0][1].get("cn")[0].decode("utf-8")
    check_host = ldap.search(f"(&(arecord={check_ip})(objectClass=univentionHost))", attr=[
                             "cn"])[0][1].get("cn")[0].decode("utf-8")
    ...
    get_portal_tile(driver, portal_config.sso_login_tile_de,
                    portal_config).click()
    wait_for_id(driver, portal_config.header_menu_id).click()
    a = wait_for_class(
        driver, portal_config.portal_sidenavigation_username_class)[0]
...
def test_every_umc_server_has_a_saml_client(ucr, keycloak_config):
    ldap = get_ldap_connection()
    umc_hosts = ldap.search(
        "univentionService=Univention Management Console", attr=["cn"])
    umc_hosts = [
        f"{host[1]['cn'][0].decode('utf-8')}.{ucr['domainname']}" for host in umc_hosts]
    ...
    kc_clients = [client["clientId"] for client in keycloak_get_request(
        keycloak_config, "realms/ucs/clients")]
...
def test_master_realm_config(keycloak_config, ucr):
    ...
    required_actions = keycloak_get_request(
        keycloak_config, "realms/master/authentication/required-actions")
    required_actions = [ra["alias"]
                        for ra in required_actions if ra["enabled"]]
    ...
    ldap_federation = keycloak_get_request(
        keycloak_config, "realms/master/components?type=org.keycloak.storage.UserStorageProvider")
    ldap_federation = next(
        lf for lf in ldap_federation if lf["name"] == "ldap-master-admin")
    ...
    components = keycloak_get_request(
        keycloak_config, f"realms/master/components?parent={ldap_federation['id']}")
...
def test_ucs_realm_config(keycloak_config, ucr):
    ...
    required_actions = keycloak_get_request(
        keycloak_config, "realms/ucs/authentication/required-actions")
    required_actions = [ra["alias"]
                        for ra in required_actions if ra["enabled"]]
    ...
    # ucs ldap federation and mapper
    ldap_federation = keycloak_get_request(
        keycloak_config, "realms/ucs/components?type=org.keycloak.storage.UserStorageProvider")
    ldap_federation = next(
        lf for lf in ldap_federation if lf["name"] == "ldap-provider")
    assert ldap_federation["providerId"] == "ldap"
    assert ldap_federation["config"]["enabled"] == ["true"]
    assert ldap_federation["config"]["editMode"] == ["READ_ONLY"]
    assert ldap_federation["config"]["allowKerberosAuthentication"] == ["true"]
    assert ldap_federation["config"]["kerberosRealm"] == [
        ucr.get("kerberos/realm")]
    if sso_fqdn:
        assert ldap_federation["config"]["serverPrincipal"] == [
            f"HTTP/{sso_fqdn}@{kerberos_realm}"]
    else:
    ...
    assert ldap_federation["config"]["keyTab"] == [
        "/var/lib/univention-appcenter/apps/keycloak/conf/keycloak.keytab"]
    components = keycloak_get_request(
        keycloak_config, f"realms/ucs/components?parent={ldap_federation['id']}")
    components = [c["name"] for c in components]
    assert set(components) == {
        "uid",
        "creation date",
...
def test_csp(keycloak_config, ucr):
    if not ucr.is_true('keycloak/server/sso/virtualhost'):
        pytest.skip(
            "no virtual host config, therefore no special cookie header settings")
    response = requests.post(keycloak_config.admin_url,
    ...
    try:
        ...
        ucr.handler_set(
            ['keycloak/csp/frame-ancestors=https://*.external.com'])
        run_command(['systemctl', 'restart', 'apache2'])
        response = requests.post(keycloak_config.admin_url, headers={
                                 "Accept": "text/html"})
        assert response.headers["Content-Security-Policy"]
