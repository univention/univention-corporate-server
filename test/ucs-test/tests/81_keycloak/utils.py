...
def keycloak_password_change(
    ...
    driver.find_element(
        By.ID, keycloak_config.password_new_id).send_keys(new_password)
    driver.find_element(By.ID, keycloak_config.password_confirm_id).send_keys(
        new_password_confirm)
    driver.find_element(
        By.ID, keycloak_config.password_change_button_id).click()
    if fails_with:
        lang = driver.execute_script(
            "return window.navigator.userLanguage || window.navigator.language")
        fails_with = fails_with[lang]
        error = driver.find_element(
            By.CSS_SELECTOR, keycloak_config.password_update_error_css_selector)
...
def keycloak_get_request(config: SimpleNamespace, path: str, params: Optional[dict] = None) -> dict:
    response = requests.get(f"{config.admin_url}/{path}",
                            headers=keycloak_auth_header(config), params=params)
    assert response.status_code == 200, response.text
    return response.json()


def keycloak_sessions(config: SimpleNamespace) -> dict:
    response = requests.get(config.client_session_stats_url,
...
def keycloak_sessions_by_user(config: SimpleNamespace, username: str) -> dict:
    ...
    response = requests.get(config.users_url, params=params,
                            headers=keycloak_auth_header(config))
    assert response.status_code == 200, response.text
    user_object = {}
    ...
    response = requests.get(
        f"{config.users_url}/{user_object['id']}/sessions", headers=keycloak_auth_header(config))
    assert response.status_code == 200, response.text
    return response.json()
...
def keycloak_login(
    ...
    if fails_with:
        fails_with = fails_with[lang]
        error = driver.find_element(
            By.CSS_SELECTOR, keycloak_config.login_error_css_selector)
...
def legacy_auth_config_remove(session: KeycloakAdmin, groups: dict) -> None:
    ...
    # remove group-mapper
    ldap_provider_id = session.get_components(
        query={"name": ldap_provider_name, "type": "org.keycloak.storage.UserStorageProvider"})[0]["id"]
    mapper = session.get_components(query={"parent": ldap_provider_id, "name": group_mapper_name,
...
def legacy_auth_config_create(session: KeycloakAdmin, ldap_base: str, groups: dict) -> None:
    ...
    if mapper:
        ...
        ldap_filter = f"(cn={group_names[0]})" if len(
            group_names) == 1 else f"(|(cn={')(cn='.join(group_names)}))"
        payload = {
    ...
    for client in groups.values():
        ...
        session.create_client_role(
            client_id, {"name": auth_role_name}, skip_exists=True)
        role = next(x for x in session.get_client_roles(
            client_id) if x["name"] == auth_role_name)
