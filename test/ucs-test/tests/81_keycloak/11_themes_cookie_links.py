...
from utils import needs_secret, run_command
...
def test_theme_switch(ucr, keycloak_adm_login, account, settings):
    theme = settings[0]
    color = settings[1]
    ucr.handler_set([f'ucs/web/theme={theme}'])
    page = keycloak_adm_login(account.username, account.bindpw, no_login=True)
...
@needs_secret
@pytest.mark.check_i_am_keycloak()
def test_custom_theme(keycloak_adm_login, account):
...
@needs_secret
@pytest.mark.check_i_am_keycloak()
def test_cookie_banner(keycloak_adm_login, account, ucr, keycloak_config):
    ucr.handler_set(
        [
...
@needs_secret
@pytest.mark.check_i_am_keycloak()
def test_cookie_banner_no_banner_with_cookie_domains(keycloak_adm_login, account, ucr):
    ...
    keycloak_adm_login(account.username, account.bindpw)


@needs_secret
@pytest.mark.check_i_am_keycloak()
def test_cookie_banner_domains(keycloak_adm_login, account, ucr, keycloak_config):
...
@needs_secret
@pytest.mark.check_i_am_keycloak()
def test_login_page_with_cookie_banner_no_element_is_tabbable(keycloak_adm_login, account, ucr):
...
@needs_secret
def test_login_page_all_elements_are_tabbable(portal_login_via_keycloak, keycloak_adm_login, account):
    page = portal_login_via_keycloak(account.username, account.bindpw, no_login=True)
...
def test_login_links(lang, link_count, login_links, portal_login_via_keycloak, account):
    page = portal_login_via_keycloak(account.username, account.bindpw, no_login=True)
