...
class TestLicenseClient(HTMLParser):
    ...
    def close_connection(self):
        ...
        if self.Connection:
            try:
                ...
                self.log.exception("An HTTP Exception occured while closing "
                                   "the connection: '%s'" % exc)
    ...
    def get_cookie(self):
        ...
        if not ('sessionid' in self.cookie):
            self.log.critical("The 'sessionid' field was not found in the "
                              "received cookie: '%s'" % self.cookie)
            exit(1)
    ...
    def make_post_request(self, url, body, headers):
        ...
        """
        self.log.debug("In 'make_post_request' method: url='%s', body='%s', "
    ...
    def handle_starttag(self, tag, attrs):
        ...
        """
        self.log.debug("In 'handle_starttag' method: tag='%s', attrs='%s'"
                       % (tag, attrs))
        if attrs:
    ...
    def get_the_license(self, body):
        ...
        try:
            ...
            self.log.exception("An exception occured while parsing the "
                               "response body for a link to a license "
                               "file: '%s'" % exc)
        if not self.link_to_license:
            self.log.critical("The link to the license file was not found in "
                              "the body from server: '%s'" % body)
            exit(1)
    ...
    def download_license_file(self):
        ...
        try:
            ...
            self.log.exception("An error happened while writing the downloaded"
                               " license to a file '%s': '%s'"
    ...
    def check_date_format(self):
        ...
        try:
            ...
            self.log.exception("The 'EndDate' for the license has a wrong "
                               "format, supported format is 'dd.mm.yyyy': %r"
                               % exc)
            exit(1)
    ...
    def update_with_parsed_args(self, args):
        ...
        if log_level:
            ...
            if isinstance(numeric_level, int):
                ...
                self.log.info("The LogLevel was not changed, unknown '%s' log "
    ...
    def process_cmd_arguments(self):
        ...
        self.Parser.add_argument("EndDate", help="The date till which the "
                                                 "license will be valid "
                                                 "(max 1 year from now)")
        self.Parser.add_argument("-f", "--FileName",
                                 help="The filename to be used for the "
                                      "issued license "
    ...
    def main(self, base_dn="", end_date="", server_url="", license_file=""):
        ...
        if self.Parser:
            ...
            self.log.error("The 'BaseDN' or/and 'EndDate' were "
                           "not provided for the license to create")
            exit(1)
