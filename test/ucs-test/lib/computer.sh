...
computer_create () { # Creates a computer. E.g. computer_create "$COMPUTERNAME"
	...
	log_and_execute udm-test "computers/$role" create --position "cn=computers,$ldap_base" --set name="$COMPUTERNAME"
}

computer_dn () { #echos the DN of a Computer. E.g. computer_dn $GROUPNAME
	local name="$1"
	udm-test computers/managedclient list --filter cn="$name" | sed -ne 's/^DN: //p'
}

computer_remove () { # Removes a computer. E.g. computer_remove "$COMPUTERNAME"
	local COMPUTERNAME=${1?:missing parameter: name}
	log_and_execute udm-test computers/managedclient remove --dn "cn=$COMPUTERNAME,cn=computers,$ldap_base"
}
