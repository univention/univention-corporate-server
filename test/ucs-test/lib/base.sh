...
_log () {
	...
	[ -n "${UCSTEST_PID:-}" ] || ts="$(date -Is) "
	shift 2
	echo -e "${ts}${_B:-<}${name}${_N:->} ${*}" >&2
}
error () { _log 0 E "$@"; }
warning () { _log 1 W "$@"; }
info () { _log 2 I "$@"; }
...
fail_bool () { # This is intended to be called directly after functions that are supposed to return 0 on successful validation, 1 on failure or anything else in case of an internal error.
	...
	# The intended calling scheme would be along the lines of: some_boolean_check || fail_test 111 "Check xxx failed" or some_boolean_check; fail_bool 1 121
	local rc=$?
