...
release='5.2-7'  #
export CURRENT_AMI="${CURRENT_AMI:=ami-08e58af159c930ff2}"  # AMI: Univention Corporate Server (UCS) 5.2 (official image) rev. 7
old_release='5.0-10'  #
export OLD_AMI="${OLD_AMI:=ami-063173dfea68712bc}"  # AMI: Univention Corporate Server (UCS) 5.0 (official image) rev. 11
export KVM_UCSVERSION="${KVM_UCSVERSION:=5.2-7+2026-09-11}"
export OPENSTACK_IMAGE_VERSION="${OPENSTACK_IMAGE_VERSION:=5.2-6}"  # version for the openstack image
...
usage () {
	...
	echo ""
	echo "Start scenario defined in <scenario.cfg> in AWS-EC2, OpenStack, KVM or Proxmox"
	echo ""
	...
	echo ""
	echo "  ucs-*-create"
	echo "    <>${BOLD}KVM_BUILD_SERVER${NORM}     - KVM build server|'EC2'|'OpenStack'|'Openstack'|'OS'|'Proxmox'|'PVE' [${BOLD}$KVM_BUILD_SERVER${NORM}]"
...
Openstack|OpenStack|OS)
	...
	;;
Proxmox|PROXMOX|PVE)
	proxmox_credentials="${UCS_PROXMOX_CREDENTIALS:-$HOME/.ucs-ec2-tools.json}"
	[ -f "$proxmox_credentials" ] ||
		die "Missing Proxmox credentials file '$proxmox_credentials' (set UCS_PROXMOX_CREDENTIALS or create ~/.ucs-ec2-tools.json)!"
	exe="ucs-proxmox-create"
...
then
	...
	# Proxmox API credentials: mount the file at the same path inside the container.
	# UCS_PROXMOX_CREDENTIALS is passed through via the env-file, so ucs-proxmox-create
	# resolves the same path; unset falls back to ~/.ucs-ec2-tools.json.
	[ "$exe" = "ucs-proxmox-create" ] &&
		cmd+=(-v "$proxmox_credentials:$proxmox_credentials:ro")
...
"$REPLACE" && cmd+=("--replace")
# ucs-proxmox-create does not know --exact-match
"$EXACT_MATCH" && [ "$exe" != "ucs-proxmox-create" ] && cmd+=("--exact-match")
