...
connection = Client(options.host, options.username, options.password)
...
result = connection.umc_command('schoolinstaller/install', params).result
...
while not status['finished']:
	if failcount >= 1200:
		...
		sys.exit(1)
	try:
		status = connection.umc_command('schoolinstaller/progress').result
		failcount = 0
	except socket.error as exc:
		failcount += 1
		print 'TRACEBACK %d in connection.umc_command("schoolinstaller/progress"):\n%s' % (failcount, traceback.format_exc(),)
		time.sleep(1)
...
result = connection.umc_command('lib/server/restart').result
if not result:
