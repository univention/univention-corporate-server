...
set_keycloak_fqdn () {
    ucr set keycloak/server/sso/fqdn="kc.$(hostname -d)"
...
register_idbroker_as_sp_in_ucs_simpleSAML () {
	...
	curl -k "https://$broker_fqdn/realms/ID-Broker/broker/$keycloak_identifier/endpoint/descriptor" > metadata.xml
	udm saml/serviceprovider create \
		...
		--set AssertionConsumerService="https://$broker_fqdn/realms/ID-Broker/broker/$keycloak_identifier/endpoint" \
		--set Identifier="https://$broker_fqdn/realms/ID-Broker/broker/$keycloak_identifier/endpoint/descriptor" \
...
wait_for_keycloak () {
	while [ ! "$(curl --location --write-out '%{http_code}' localhost:8180 -o /dev/null 2> /dev/null)" -eq 200 ]; do
