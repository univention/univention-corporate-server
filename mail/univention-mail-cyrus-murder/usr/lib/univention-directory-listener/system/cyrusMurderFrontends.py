	...
	try:
		if new and new.has_key('mailPrimaryAddress'):
			...
			if configRegistry.has_key('mail/cyrus/murder/frontends'):
				if not emailId in configRegistry['mail/cyrus/murder/frontends'].split(' '):
					listener.run('/usr/sbin/univention-config-registry', ['univention-config-registry','set', 'mail/cyrus/murder/frontends=%s %s' % ( configRegistry['mail/cyrus/murder/frontends'], emailId)], uid=0)
			else:
				listener.run('/usr/sbin/univention-config-registry', ['univention-config-registry','set', 'mail/cyrus/murder/frontends=%s' % (emailId)], uid=0)
		elif old and old.has_key('mailPrimaryAddress'):
			emailId = "%s" % old['mailPrimaryAddress'][0]
			if emailId in configRegistry['mail/cyrus/murder/frontends'].split(' '):
				listener.run('/usr/sbin/univention-config-registry', ['univention-config-registry','set', 'mail/cyrus/murder/frontends=%s' % ( configRegistry['mail/cyrus/murder/frontends'].replace(emailId, ''))], uid=0)
