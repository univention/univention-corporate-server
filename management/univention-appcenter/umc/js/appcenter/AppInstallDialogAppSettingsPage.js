...
], function(declare, lang, array, domConstruct, ContentPane, tools, Text, ComboBox, Form, put, _) {
	return {
		getPageConf: function(info, isMainInfo, isMultiAppInstall) {
			if (!info.appSettingsFormConf) {
				return null;
			}
			...
			var name = lang.replace('appSettings_{0}', [info.app.id]);
			return {
				name: name,
				...
				widgets: [{
					...
					type: Form,
					'class': 'appInstallDialog__readme appInstallDialog__appSettings',
					name: lang.replace('{0}_appSettings', [name]),
					widgets: info.appSettingsFormConf.widgets,
					layout: info.appSettingsFormConf.layout
				}]
			};
		}
	};
