...
], function(declare, lang, domConstruct, ContentPane, entities, tools, Text, _) {
	return {
		getPageConf: function(info, isMainInfo, isMultiAppInstall) {
			if (!info.app.licenseAgreement) {
				return null;
			}
			...
			var name = lang.replace('licenseAgreement_{0}', [info.app.id]);
			return {
				name: name,
				...
				widgets: [{
					...
					content: _('License agreement')
				}, {
					type: Text,
					'class': 'appInstallDialog__readme',
					name: lang.replace('{0}_licenseAgreement', [name]),
					content: info.app.licenseAgreement
				}]
			};
		}
	};
