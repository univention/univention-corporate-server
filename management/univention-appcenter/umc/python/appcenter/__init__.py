            ...
            for app in apps:
                for host in hosts:
                    ...
                    if action == 'upgrade' and app.id in auto_installed:
                        app_action = 'install'
                    if host == localhost:
                        host_result[app.id] = self._run_local(app, app_action, _settings, auto_installed, progress)
                    else:
                        host_result[app.id] = self._run_remote(request, host, app, app_action, auto_installed, _settings, progress)[app.id]
    ...
    def _get_config(self, app, phase):
        ...
        for setting in app.get_settings():
            if phase in setting.show or phase in setting.show_read_only:
                if setting.name in new_values:
                    value = new_values[setting.name]
                else:
