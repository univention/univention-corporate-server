...
class Upgrade(Install):
    ...
    def initial_values_of_new_settings(cls, app, old_app):
        """Initialize newly declared settings that a fresh installation sets, preserving their stored values."""
        if old_app is None or app == old_app:
            return {}
        old_names = {setting.name for setting in old_app.get_settings()}
        # The new version is not marked installed yet, so its object cannot see the running container.
        read_app = app if app.is_installed() else old_app
        values = {}
        for setting in app.get_settings():
            ...
            if not app.docker and 'Install' not in setting.show:
                continue
            try:
                value = setting.get_value(read_app, preserve_unset=True)
                if value is None:
    ...
    def _get_configure_settings(self, app, filter_action=True):
        ...
        set_vars.update(self.initial_values_of_new_settings(app, self.old_app))
        return set_vars
