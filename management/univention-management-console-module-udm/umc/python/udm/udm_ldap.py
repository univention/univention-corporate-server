        ...
        try:
            ...
            if simple and self.allows_simple_lookup():
                ...
                if lookup_filter is None:
                    ...
                    context = {'module': self.name, 'custom_filter': filter, 'value': value, 'allow_asterisks': allow_asterisks, 'default_attributes': self.default_search_attrs}
                    if simple_attrs is not None:
                        result = ldap_connection.search_filtered(context, filter=str(lookup_filter), base=container, scope=scope, sizelimit=sizelimit, attr=simple_attrs, serverctrls=serverctrls, response=response)
                    else:
                        result = ldap_connection.search_dn_filtered(context, filter=str(lookup_filter), base=container, scope=scope, sizelimit=sizelimit, serverctrls=serverctrls, response=response)
            else:
                if self.module:
                    kwargs = {}
