	...
	return {
		...
		_showRequestTokenMessage: function(res) {
			...
			if (!res.success) {
				...
				msg = _('Hello <b>%s</b>, we have sent you an email to <b>%s</b>. Please follow the instructions in the email to verify your account.', entities.encode(res.data.username), entities.encode(res.data.email));
				msg = domConstruct.toDom(msg);
				msg = put('p', msg);
				this._setMessage(msg);
			}
