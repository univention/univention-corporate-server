...
* No LDAP ACL's **strategic decision**
...
* Probably a future need for "value-based" authorization (e.g. `username` can not be `root`, group memberships can not be `Domain Admins`, certain `guadianRoles` are allowed)
...
# Integration of UDM Authoriztion into the Guardian concept
...
- Capabilities for general actions of a all UDM modules (if the module supports the action):
  ...
  - Capability: condition `objectType == "users/user"` grants the permission `udm:{module}:reports-type-query` allows to query the available report types of this module
...
- `udm:default-roles:domain-users` allows to read everything non-sensible
...
**Example2:**: A easy to understand, human read- and writeable [format](example.acl) inspired by LDAP ACLs:
```
named-condition "same-ou"
  condition="udm:udm_target_position_from_context"
  parameters context="udm:contexts:ou" scope="subtree"

named-condition "in-global-users-container"  # unused example
  condition="udm:udm_target_position_in"
...
access by role="udm:default-roles:organizational-unit-administrator" context="udm:contexts:ou"
  ...
  to objecttype="users/user" actions="create,modify,rename,remove,move"
  ...
  to objecttype="groups/group" actions="create,modify,rename,remove,move"
  ...
  to objecttype="groups/group" properties="guardianMemberRoles" if="same-ou" values=["udm:default-roles:domain-users", …] grant-access=none
...
We has a support case which took weeks to solve because Keycloak couldn't be reached.
