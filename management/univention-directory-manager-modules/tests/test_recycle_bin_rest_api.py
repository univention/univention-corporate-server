...
class TestRecycleBinRESTAPI(TestCase):
    ...
    def test_purge_specific_dns(self):
        ...
        assert result.get("success"), "Purge operation was not successful"
        assert len(result.get("purged", [])) == 2, "Expected 2 entries to be purged"

        assert self.count_entries(['test-api-1', 'test-api-2']) == 0, \
    ...
    def test_purge_invalid_dns(self):
        ...
        assert error_data.get("error_id") == "PURGE_FAILED"
        assert "affected_items" in error_data
    ...
    def test_invalid_request_body(self):
        ...
        assert response.status_code == 400, "Should reject missing 'dns' field"
        ...
        assert response.status_code == 400, "Should reject non-list 'dns' field"
        ...
        assert response.status_code == 400, "Should reject empty 'dns' list"
    ...
    def test_dry_run_mode(self):
        ...
        assert self.count_entries(['test-api-dryrun']) == 0, \
