...
class RecycleBinListener(ListenerModuleHandler):
    ...
    def _populate_cache(self):
        ...
        if ucr.get('server/role') != 'domaincontroller_master':
            self.logger.debug('Not master, skipping cache population')
            return

        # make sure recyclebin container exists
        # TODO: maybe remove before release, we need it now for the tests
        if not self.admin_lo.get(RECYCLEBIN_BASE):
            mod = univention.admin.modules.get('container/cn')
            position = univention.admin.uldap.position('cn=internal')
            obj = mod.object(None, self.admin_lo, position)
            obj.open()
            obj['name'] = 'recyclebin'
            obj['description'] = 'Container for deleted LDAP objects (recyclebin)'
            obj.create()
    ...
    def _handle_group_membership_changes(self, group_dn, new_attrs, old_attrs):
        ...
        for member_dn in removed_member_dns:
            ...
            if deleted_object_attrs and 'univentionRecycleBinOriginalType' in deleted_object_attrs:
                ...
                refs = [
                    bytes(ref) for ref in create_references(self.admin_lo, object_type, {'memberOf': [group_dn.encode('UTF-8')]})
