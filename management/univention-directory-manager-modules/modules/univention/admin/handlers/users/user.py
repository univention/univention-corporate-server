...
def mapUTCDateTimeToLocaltime(localtime):  # type: (time.struct_time) -> List[bytes]
	if localtime:
		epoch_seconds = time.mktime(localtime)
		utctime = time.gmtime(epoch_seconds)
		return [utctime.strftime("%Y%m%d%H%M%SZ")]
	return []


def unmapUTCDateTimeToLocaltime(attribute_value):  # type: (List[bytes]) -> Union[time.struct_time, None]
	if attribute_value and attribute_value[0]:
		generalizedtime = int(attribute_value[0])
		try:
...
mapping.register('accountActivationDate', 'krb5ValidStart', mapLocaltimeToUTCDateTimeString, unmapUTCDateTimeToLocaltime)
