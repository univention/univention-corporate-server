...
class object(univention.admin.handlers.simpleLdap, PKIIntegration, GuardianBase):
    ...
    def actions(self):
        data = super().actions
        data.update({
            'unlock': univention.admin.action(
                short_description=_('Unlock user'),
                long_description=_('Unlock user'),
                execute_post=self._action_unlock,
                is_enabled=lambda obj: obj['locked'] == '1',
                show_as_button=True,
            ),
            'disable': univention.admin.action(
                short_description=_('Disable user'),
                long_description=_('Disable user'),
                execute_post=self._action_disable,
                show_as_button=True,
            ),
            'service-specific-password': univention.admin.action(
                short_description=_('Generate service specific password'),
                long_description=_('Generate a new service specific password'),
                execute_post=self._action_service_specific_password,
            ),
        })
        return data
    ...
    @classmethod
    def _action_unlock(cls, obj):
        """Unlock the user account"""
        obj['unlock'] = '1'
        obj.modify()

    @classmethod
    def _action_disable(cls, obj):
        """Disable the user account"""
        obj['disabled'] = '1'
        obj.modify()

    @classmethod
    def _action_service_specific_password(cls, obj, *, service: str | None = None):
        """Let a new autogenerated service specific password be created"""
        cfg = univention.password.password_config(service)
        new_password = univention.password.generate_password(**cfg)

        obj['serviceSpecificPassword'] = {'service': service, 'password': new_password}
        obj.modify()

        return {'service': service, 'password': new_password}
