...
class ComputerObject(univention.admin.handlers.simpleComputer, nagios.Support, PKIIntegration, GuardianBase, appcenter.AppHost):
    ...
    def open(self) -> None:
        ...
        if self.exists():
            if 'posix' in self.options and not self.info.get('primaryGroup'):
                ...
                log.debug({'msg': 'open primary group number', 'value': primaryGroupNumber})
                if primaryGroupNumber:
                    ...
                    if primaryGroupResult:
                        ...
                        log.debug({'msg': 'open: set primary group', 'value': self['primaryGroup']})
                    else:
    ...
    def unmapped_lookup_filter(cls) -> univention.admin.filter.conjunction:
        filter_p = super().unmapped_lookup_filter()
    ...
    def identify(cls, dn: str, attr: univention.admin.handlers._Attributes, canonical: bool = False) -> bool:
        if cls.SERVER_ROLE and cls.SERVER_ROLE != 'member' and cls.SERVER_ROLE not in [x.decode('UTF-8') for x in attr.get('univentionServerRole', [])]:
            return False
        return super().identify(dn, attr, canonical)
