...
layout=[
	univention.admin.tab(_('General'),_('Basic Values'),[
		...
		[univention.admin.field('listWizards'), univention.admin.field('listModules')],
	]),
]
...
class object(univention.admin.handlers.simpleLdap):
	...
	def _ldap_pre_create(self):
		self.dn='cn=admin-settings,cn=univention,%s' % (self.position.getDomain())

	def _ldap_addlist(self):
		return [('objectClass', ['top', 'univentionAdminGlobalSettings'] ) ]

def lookup(co, lo, filter_s, base='', superordinate=None, scope='sub', unique=0, required=0, timeout=-1, sizelimit=0):

	filter=univention.admin.filter.conjunction('&', [
		univention.admin.filter.expression('objectClass', 'univentionAdminGlobalSettings')
		])
...
def identify(dn, attr, canonical=0):
	...
	return 'univentionAdminGlobalSettings' in attr.get('objectClass', [])
