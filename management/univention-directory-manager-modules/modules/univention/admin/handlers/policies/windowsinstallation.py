...
#  admin policy for the windows installation
#
...
policy_oc='univentionPolicyWindowsInstallation'
policy_apply_to=["computers/windows"]
...
layout = [
	Tab(_('General'),_('Windows installation'), layout = [
		'name',
		'unattendFile',
		] ),
...
mapping.register('unattendFile', 'univentionWindowsInstallationUnattendFile', None, univention.admin.mapping.ListToString)
...
class object(univention.admin.handlers.simplePolicy):
	...
	def _ldap_addlist(self):
		return [ ('objectClass', ['top', 'univentionPolicy', 'univentionPolicyWindowsInstallation']) ]
	
def lookup(co, lo, filter_s, base='', superordinate=None, scope='sub', unique=0, required=0, timeout=-1, sizelimit=0):

	filter=univention.admin.filter.conjunction('&', [
		univention.admin.filter.expression('objectClass', 'univentionPolicyWindowsInstallation')
		])
...
def identify(dn, attr, canonical=0):
	return 'univentionPolicyWindowsInstallation' in attr.get('objectClass', [])
