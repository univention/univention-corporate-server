...
policy_oc='univentionPolicyConsoleAccess'
...
mapping.register('allow', 'univentionConsoleAllow')
mapping.register('disallow', 'univentionConsoleDisallow')
...
class object(univention.admin.handlers.simplePolicy):
	...
	def _ldap_addlist(self):
		return [ ('objectClass', ['top', 'univentionPolicy', 'univentionPolicyConsoleAccess']) ]

def lookup(co, lo, filter_s, base='', superordinate=None, scope='sub', unique=0, required=0, timeout=-1, sizelimit=0):

	filter=univention.admin.filter.conjunction('&', [
		univention.admin.filter.expression('objectClass', 'univentionPolicyConsoleAccess')
		])
...
def identify(dn, attr, canonical=0):
	return 'univentionPolicyConsoleAccess' in attr.get('objectClass', [])
