...
class object(simpleLdap):
    ...
    def _unmap_original_properties(self, info, oldattr):
        if self.dn and 'originalObjectType' in info:
            ...
            try:
                mod = module.object(None, self.lo, None)
                props = mod.mapping.unmapValues(oldattr)
                props = mod._post_unmap(props, oldattr)
            ...
            props.update(mod.info)

            # add original properties to description
            # for prop in props:
            for prop in mod.descriptions:
                if prop in mod.descriptions and prop not in self.descriptions:
                    ...
                    self.descriptions[prop] = copy.deepcopy(mod.descriptions[prop])  # don't change original description!
    ...
    def get_references(self):
        return create_references(self.lo, self.info['originalObjectType'], self.oldattr)
