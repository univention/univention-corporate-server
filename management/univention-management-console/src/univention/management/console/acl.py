...
class LDAP_ACLs(ACLs):
    ...
    def _read_from_ldap(self, lo):
        if self.userdn is None:  # local users like root, __systemsetup__
            self._read_from_file(self.username)
            return
        ...
        try:
            ...
            ACL.warning('Error reading credentials from LDAP for user', username=self.username, dn=self.userdn, exc_info=True)
            # read ACLs from file
            self._read_from_file(self.username)
            return
    ...
    def _read_from_guardian(self, lo):
        ...
        try:
            ...
            ACL.warning('Error reading server object from LDAP', username=self.username, dn=self.userdn, exc_info=True)
            # read ACLs from file
            self._read_from_file(self.username)
            return
        attributes = obj.info
