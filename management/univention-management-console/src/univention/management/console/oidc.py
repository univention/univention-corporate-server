...
from univention.management.console.error import (
    BadRequest, NotFound, ServerError, ServiceUnavailable, UMC_Error, Unauthorized,
)
...
class OIDCUser(object):
    ...
    def __init__(self, id_token, access_token, refresh_token, claims):
        ...
        self.session_end_time = int(monotonic() + (claims['exp'] - time.time()))
...
class OIDCResource(OAuth2Mixin, Resource):
    ...
    async def authenticate(self, code, code_verifier, nonce):
        ...
        try:
            ...
            raise ServiceUnavailable(self._('Could not receive token from authorization server.'))
        ...
        try:
            ...
            raise ServerError(self._("Authorization server response did not contain token."))
    ...
    async def get_user_information(self, bearer_token):
        ...
        try:
            ...
            raise ServerError(self._("Could not receive user information from OP."))
...
class OIDCLogin(OIDCResource):
    ...
    async def do_single_sign_on(self, location, login_hint):
        ...
        shared_memory.pkce[state] = {'iss': self.op, 'location': location, 'code_verifier': code_verifier, 'nonce': nonce}
...
class OIDCLogoutBase(OIDCResource):
...
class OIDCLogout(OIDCLogoutBase):
...
class OIDCLogoutFinished(OIDCLogoutBase):

    def get(self):
        self._logout_success()


class OIDCFrontchannelLogout(OIDCLogoutBase):
...
class OIDCBackchannelRefresh(OIDCResource):
    """Refresh the tokens using the refresh token"""

    async def get(self):
        # TODO: restrict how often this can be called
        user = self.current_user
        if not user or not user.oidc or not user.oidc.refresh_token:
            raise Unauthorized(self._("No refresh token available."))
        ...
        self.content_negotiation(None, wrap=False)
...
if __name__ == '__main__':
    ...
    with open(oidc['op_file']) as fd:
        op = json.load(fd)
    with open(oidc['jwks_file']) as fd:
        jwks = json.load(fd)
