...
class AuthHandler:
    ...
    def __authenticate_thread(self, pam, username, password, new_password, auth_type=None, **custom_prompts):
        ...
        try:
            if new_password:
                self.__authenticate_ldap(pam, username, password)
            else:
    ...
    def __authenticate_ldap(self, pam, username, password):
        ...
        try:
            ...
            AUTH.warning('Failed to authenticate user %s via LDAP: %s', username, exc)
            raise AuthenticationFailed(pam._('The authentication has failed, please login again.'))
    ...
    def __change_password_udm(self, username, new_password):
        ...
        if lo:
            ...
            try:
                ...
                AUTH.exception('__change_password_udm(): failed to set password:')
