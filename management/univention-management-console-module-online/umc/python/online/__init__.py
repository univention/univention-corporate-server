...
_ = umc.Translation('univention-management-console-module-online').translate
...
STATUS_ICONS = {
	'installed':		'online-installed',
	'available':		'online-available',
	'access_denied':	'online-access-denied'
}
DEFAULT_ICON = 'online-unknown'		# any states not defined above
...
class Instance(umcm.Base):
	...
	def init(self):
		try:
			...
			MODULE.info("Initializing 'online' module (PID = %d, LANG = '%s')" % (getpid(),self.locale))

			MODULE.info("(not defining workaround for translation)")
#			global _
#			_ = umc.Translation('univention-management-console-module-online',self.locale).translate
	...
	def query_components(self,request):
		...
		"""
		# ----------- DEBUG -----------------
		MODULE.info("online/components/query invoked with:")
		...
		# ----------- DEBUG -----------------
		MODULE.info("online/components/query returns: %d entries" % len(result))
		pp = pprint.PrettyPrinter(indent=4)
	...
	def query_releases(self,request):
		...
		# ----------- DEBUG -----------------
		MODULE.info("online/updates/query invoked with:")
		pp = pprint.PrettyPrinter(indent=4)
		st = pp.pformat(request.options).split("\n")
		for s in st:
				MODULE.info("   << %s" % s)
		...
		# ----------- DEBUG -----------------
		MODULE.info("online/updates/query returns: %d entries" % len(result))
		pp = pprint.PrettyPrinter(indent=4)
		st = pp.pformat(result).split("\n")
		for s in st:
				MODULE.info("   >> %s" % s)
	...
	def get_components(self,request):
		# ----------- DEBUG -----------------
		MODULE.info("online/components/get invoked with:")
		pp = pprint.PrettyPrinter(indent=4)
		st = pp.pformat(request.options).split("\n")
		for s in st:
				MODULE.info("   << %s" % s)
		...
		# ----------- DEBUG -----------------
		MODULE.info("online/components/get returns:")
		pp = pprint.PrettyPrinter(indent=4)
		st = pp.pformat(result).split("\n")
		for s in st:
				MODULE.info("   >> %s" % s)
	...
	def del_components(self,request):
		# ----------- DEBUG -----------------
		MODULE.info("online/components/del invoked with:")
		pp = pprint.PrettyPrinter(indent=4)
		st = pp.pformat(request.options).split("\n")
		for s in st:
				MODULE.info("   << %s" % s)
...
#				result['message'] = "Writing UCR failed: %s" % str(ex)
		...
		# ----------- DEBUG -----------------
		MODULE.info("online/components/del returns:")
		pp = pprint.PrettyPrinter(indent=4)
		st = pp.pformat(result).split("\n")
		for s in st:
				MODULE.info("   >> %s" % s)
	...
	def put_components(self,request):
		...
		"""
		# ----------- DEBUG -----------------
		MODULE.info("online/components/put invoked with:")
		pp = pprint.PrettyPrinter(indent=4)
		st = pp.pformat(request.options).split("\n")
		for s in st:
				MODULE.info("   << %s" % s)
...
#				result['message'] = "Writing UCR failed: %s" % str(ex)
		...
		# ----------- DEBUG -----------------
		MODULE.info("online/components/put returns:")
		pp = pprint.PrettyPrinter(indent=4)
		st = pp.pformat(result).split("\n")
		for s in st:
				MODULE.info("   >> %s" % s)
	...
	def updates_available(self,request):
		...
		"""
		# ----------- DEBUG -----------------
		MODULE.info("online/updates/available invoked with:")
		pp = pprint.PrettyPrinter(indent=4)
		st = pp.pformat(request.options).split("\n")
		for s in st:
				MODULE.info("   << %s" % s)
		...
		# ----------- DEBUG -----------------
		MODULE.info("online/updates/available returns:")
		pp = pprint.PrettyPrinter(indent=4)
		st = pp.pformat(result).split("\n")
		for s in st:
				MODULE.info("   >> %s" % s)
	...
	def status(self,request):
		...
		# ----------- DEBUG -----------------
		MODULE.info("online/status invoked with:")
		pp = pprint.PrettyPrinter(indent=4)
		st = pp.pformat(request.options).split("\n")
		for s in st:
				MODULE.info("   << %s" % s)
		...
		# ----------- DEBUG -----------------
		MODULE.info("online/status returns:")
		pp = pprint.PrettyPrinter(indent=4)
	...
	def get_settings(self,request):
		...
		"""
		# ----------- DEBUG -----------------
		MODULE.info("online/settings/get invoked with:")
		pp = pprint.PrettyPrinter(indent=4)
		st = pp.pformat(request.options).split("\n")
		for s in st:
				MODULE.info("   << %s" % s)
		...
		# ----------- DEBUG -----------------
		MODULE.info("online/settings/get returns:")
		pp = pprint.PrettyPrinter(indent=4)
		st = pp.pformat(result).split("\n")
		for s in st:
				MODULE.info("   >> %s" % s)
	...
	def put_settings(self,request):
		...
		# ----------- DEBUG -----------------
		MODULE.info("online/settings/put invoked with:")
		pp = pprint.PrettyPrinter(indent=4)
		st = pp.pformat(request.options).split("\n")
		for s in st:
				MODULE.info("   << %s" % s)
		...
		# ----------- DEBUG -----------------
		MODULE.info("online/settings/put returns:")
		pp = pprint.PrettyPrinter(indent=4)
		st = pp.pformat(result).split("\n")
		for s in st:
				MODULE.info("   >> %s" % s)
	...
	def running(self,request):
		...
		"""
		# ----------- DEBUG -----------------
		MODULE.info("online/installer/running invoked with:")
		pp = pprint.PrettyPrinter(indent=4)
		st = pp.pformat(request.options).split("\n")
		for s in st:
				MODULE.info("   << %s" % s)
		...
		# ----------- DEBUG -----------------
		MODULE.info("online/installer/running returns:")
		pp = pprint.PrettyPrinter(indent=4)
		st = pp.pformat(result).split("\n")
		for s in st:
				MODULE.info("   >> %s" % s)
	...
	def updater_log_file(self,request):
		...
		"""
		# ----------- DEBUG -----------------
		MODULE.info("online/installer/logfile invoked with:")
		pp = pprint.PrettyPrinter(indent=4)
		st = pp.pformat(request.options).split("\n")
		for s in st:
				MODULE.info("   << %s" % s)
		...
		# ----------- DEBUG -----------------
		MODULE.info("online/installer/logfile returns:")
		pp = pprint.PrettyPrinter(indent=4)
		st = pp.pformat(result).split("\n")
		for s in st:
				MODULE.info("   >> %s" % s)
	...
	def updater_job_status(self,request):
		...
		"""
		# ----------- DEBUG -----------------
		MODULE.info("online/installer/status invoked with:")
		pp = pprint.PrettyPrinter(indent=4)
		st = pp.pformat(request.options).split("\n")
		for s in st:
				MODULE.info("   << %s" % s)
