...
dojo.provide("umc.modules._online.ProgressPage");

dojo.require("umc.i18n");
dojo.require("umc.dialog");

dojo.require("umc.modules._online.Page");
dojo.require("umc.modules._online._LogViewer");
...
//		and umc.modules._online._PollingMixin.
...
//
dojo.declare("umc.modules._online.ProgressPage", umc.modules._online.Page, {
    ...
    buildRendering: function() {
    	...
    	this._log = new umc.modules._online._LogViewer({
    		i18nClass:		this.i18nClass,
    		moduleStore:	this.moduleStore,
    		region:			'center',
    		query:			'online/installer/logfile'
    	});
    	...
    	this._close = new umc.widgets.Button({
    		...
    		onClick:	dojo.hitch(this, function() {
    			...
    			if (this.last_tab)
    			{
   				...
   				this.stopWatching(tab);		// online Module listens here and will switch display back to the given tab.
    		})
    	});
	...
	_query_job_status: function() {
		...
		if (this._job_key == '')
		{
			this.moduleStore.umcpCommand('online/installer/running',{},false).then(
				dojo.hitch(this, function(data) {
					this._query_success('online/installer/running');
					this._process_job_status(data);
				}),
				dojo.hitch(this, function(data) {
					this._query_error('online/installer/running',data);		// handles error
		...
		{
			this.moduleStore.umcpCommand('online/installer/status',{job:this._job_key},false).then(
					dojo.hitch(this, function(data) {
						this._query_success("online/installer/status(" + this._job_key + ")");
						this._process_job_status(data);
					}),
					dojo.hitch(this, function(data) {
						this._query_error('online/installer/status',data);		// handles error
						this._process_job_status(null);							// only for rescheduling
					})
				);
		}
	...
	// that a job has been started. The online Module listens here and will then
