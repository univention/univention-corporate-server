...
dojo.provide("umc.modules._online.UpdatesPage");
    	...
    	[
    	 	...
    	 	// -------------------- Release updates --------------------------
			{
				...
				name:			'releases',
		 		...
		 		dynamicValues:	'online/updates/query',
		 		// FIXME Manual placement: should be done by the layout framework some day.
		 		style:			'width:300px;',
		 		onValuesLoaded:	dojo.hitch(this, function(values) {
		 			try
		 			{
		 				this._query_success('online/updates/query');
...
//           	},
    	...
    	dojo.connect(this._log,'_query_success',dojo.hitch(this,function(subject) {
       	...
       	dojo.connect(this._form,'onLoaded',dojo.hitch(this, function(args) {
       		try
       		{
       			this._query_success('online/updates/get');
	...
	_check_dist_upgrade: function() {
		...
		{
			this.standby(true);
			this.moduleStore.umcpCommand('online/updates/available').then(
	...
	_reboot: function() {
		...
		this.standby(true);
		this.moduleStore.umcpCommand('online/installer/reboot').then(dojo.hitch(this, function() {
			this.standby(false);
	...
	// These functions are stubs that the 'online' Module is listening to,
