...
dojo.provide("umc.modules._online.ComponentsPage");
...
dojo.declare("umc.modules._online.ComponentsPage", umc.modules._online.Page, {
...
//            	iconClass:			'dijitIconPackage',
    	...
    	this._grid = new umc.modules._online.Grid({
    		region:			'center',
    		query:			this._query,
			...
			polling:	{
				interval:	5000,
				query:		'online/components/serial',
				callback:	dojo.hitch(this, function() {
					this.refresh(true);
				})
			}
	...
	// overload our stub from umc.modules._online.Page
	...
	_enable_component: function(name,enabled) {
		...
		{
			this.standby(true);
			this.moduleStore.umcpCommand('online/components/put',args).then(
	...
	_delete_components: function(ids) {
		...
		[
		 	...
		 	},
		 	{
		 		...
		 		callback:	dojo.hitch(this,function() {
		 			this.standby(true);
		 			this.moduleStore.umcpCommand('online/components/del',ids).then(
		 					dojo.hitch(this, function(data) {
		 						this.standby(false);
		 						this.refresh();
		 						this.dataChanged();
