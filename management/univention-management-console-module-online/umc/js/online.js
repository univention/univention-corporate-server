...
dojo.provide("umc.modules.online"); 
...
// ------- Overloaded classes --------
dojo.require("umc.modules._online.Module");
dojo.require("umc.modules._online.Page");
dojo.require("umc.modules._online.Grid");
dojo.require("umc.modules._online.Form");

// ------- Pages inside the module ----------
dojo.require("umc.modules._online.UpdatesPage");
dojo.require("umc.modules._online.ComponentsPage");
dojo.require("umc.modules._online.DetailsPage");
dojo.require("umc.modules._online.SettingsPage");
dojo.require("umc.modules._online.ProgressPage");

dojo.declare("umc.modules.online", umc.modules._online.Module, {
	...
	buildRendering: function() {
		...
		this._updates = new umc.modules._online.UpdatesPage({
			moduleStore:	umc.store.getModuleStore(null,'online/updates')
		});
		
		this._components = new  umc.modules._online.ComponentsPage({
			moduleStore:	umc.store.getModuleStore('name','online/components')
		});

		this._details = new umc.modules._online.DetailsPage({
			moduleStore:	umc.store.getModuleStore('name','online/components')
			
		});
		
		this._settings = new umc.modules._online.SettingsPage({
			moduleStore:	umc.store.getModuleStore('dummy','online/settings')
		});

		this._progress = new umc.modules._online.ProgressPage({
			// Strictly spoken, we don't need the args to this moduleStore,
			// we simply need the store itself.
			moduleStore:	umc.store.getModuleStore(null,'online/installer')
		});
	...
	_confirm_distupgrade: function() {
		...
		{
			this.standby(true);
			this.moduleStore.umcpCommand('online/updates/check').then(dojo.hitch(this, function(data) {
				this.standby(false);
	...
	// and detail are passed as args to the 'online/installer/execute' backend.
	//
	...
	_call_installer: function(args) {
		...
		this.moduleStore.umcpCommand('online/installer/execute',{
			job:	args['job'],
