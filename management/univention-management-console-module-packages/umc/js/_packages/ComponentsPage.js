...
dojo.provide("umc.modules._packages.ComponentsPage");
...
dojo.declare("umc.modules._packages.ComponentsPage", [ umc.widgets.Page, umc.widgets.StandbyMixin, umc.i18n.Mixin ], {
...
//				iconClass: 'umcIconRefresh',
	...
	_enable_component: function(name, enabled) {
		...
		if (args.length) {
			this.standby(true);
			umc.tools.umcpCommand('packages/components/put', args).then(
	...
	_delete_components: function(ids) {
		...
		[
			...
			},
			{
				...
				callback: dojo.hitch(this, function() {
					this.standby(true);
					umc.tools.umcpCommand('packages/components/del', ids).then(
							dojo.hitch(this, function(data) {
								this.standby(false);
								this.refresh();
							}),
