...
warning_message = _('The memberOf module is not enabled in the LDAP server on this host. This feature is activated by default with UCS version 4.3-0 and serveral services depend on it. You may want to consider activating the memberOf module for the LDAP server according to:')
links = [{
...
def run(_umc_instance):
	if util.is_service_active('LDAP'):
		if not ucr.is_true('lldap/overlay/memberof'):
