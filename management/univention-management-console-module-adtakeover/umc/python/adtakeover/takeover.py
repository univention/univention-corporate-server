...
class UCS_License_detection():
	...
	def determine_license(self, lo, dn):
		...
		num = [ mylen (obj)
			for obj in objs]
		expired = self._license.checkObjectCounts(max, num)
		result = []
		for i in types.keys():
			t = types[i]
			m = max[i]
			...
			if m:
				if i == self.License.USERS or i == self.License.ACCOUNT:
					log.debug("determine_license for current UCS %s: %s of %s" % (l, n, m))
...
class AD_Takeover():
	...
	def time_sync(self, tolerance=180, critical_difference=360):
		...
		env["LC_ALL"] = "C"
		try:
			p1 = subprocess.Popen(["rdate", "-p", "-n", self.ad_server_ip],
