...
def handler(dn, new, listener_old):
	...
	try:
		...
		if new and not old:
			al=addlist(new)
			univention.debug.debug(univention.debug.LISTENER, univention.debug.ALL, 'add: %s' % univention.utf8.decode(dn))
			try:
				l.add_s(dn, al)
			except ldap.OBJECT_CLASS_VIOLATION, msg:
				univention.debug.debug(univention.debug.LISTENER, univention.debug.ERROR, 'replication: object class violation while adding %s' % univention.utf8.decode(dn))
		# delete
		elif old and not new:
			univention.debug.debug(univention.debug.LISTENER, univention.debug.ALL, 'delete: %s' % univention.utf8.decode(dn))
			try:
				l.delete_s(dn)
		...
		# modify
			...
			ml=modlist(old, new)
			if ml:
				univention.debug.debug(univention.debug.LISTENER, univention.debug.ALL, 'modify: %s' % univention.utf8.decode(dn))
				l.modify_s(dn, ml)
