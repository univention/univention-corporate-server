...
/**
 * @module portal/FolderPropertiesDialog
 */
define([
	"dojo/_base/declare",
	"dojo/_base/lang",
	...
	const FolderPropertiesForm = declare("FolderPropertiesForm", [Form], {
		...
		save: function() {
			...
			return tools.umcpCommand('udm/validate', {
				...
				portalContent.modify('folder', moduleStoreFunc, moduleStoreParams, moduleStoreOptions, this.parentDn).then(() => {
					this.hide();
				}, () => {
					this.standby(false);
				});
	...
	return declare("FolderPropertiesDialog", [Dialog], {
		//// overwrites
		...
		load: async function() {
			this.standby(true);
			const formConf = await properties.getFormConf('portals/folder', ['name', 'displayName', 'entries'],
					this.folder ? this.folder.dn : null);
			const form = new FolderPropertiesForm({
				...formConf,
				parentDn: this.parentDn,
				standby: lang.hitch(this, 'standby'),
				hide: lang.hitch(this, 'hide'),
			});
			on(form, 'noChanges', lang.hitch(this, 'onNoChanges'));
			this._form = form;
			if (this.folder) {
				await form.load(this.folder.dn);
			}
		...
		remove() {
			this.standby(true);
			portalContent.removeEntry(this.parentDn, this.folder.idx).then(() => {
				this.hide();
			}, () => {
				this.standby(false);
			});
