# UDM HTTP API in a RESTful architecture

The UDM REST API is a JSON-HTTP interface to the Univention Directory Manager (UDM).
It tries to be as RESTful as possible with the trade off that it e.g. additionaly provides OpenAPI schema.
Conforming to the REST principles is mainly for realizing an easy future web UI interface
but also for the reference client implementation (`univention.admin.rest.client`) which is used for machine-to-machine communication.
Following the principles will allow us to make future changes to the UDM REST API without breaking backwards compatibility.
We also provide API stability for the OpenAPI schema, when the latest schema is used.

## RESTful

**RE**presentation **S**tate **T**ransfer is an architectural style defined in Roy T. Fieldings dissertation about
...
REST is an hybrid style of best practice network styles and consists of 6 architectural constraints and 4 interface constraints.
Conforming to all constraints makes a service *RESTful*.
...
1. Client-Server
    ...
    **UDM REST API Compliance**: The UDM REST API accomplishes this by providing hypermedia controls through *Link* relations, included in Hypertext Application Language (HAL) response representations, as well as "Link" HTTP response headers.
...
2. Stateless
    ...
    **UDM REST API Compliance**: The UDM REST API accomplishes this by not storing any state across requests and utilyzing HTTP basic authentication for user authentication. Additionally, authentication via OIDC/Bearer will be provided in the future, which is compliant to the statelessness constraint.
...
3. Cache
    ...
    **UDM REST API Compliance**: The UDM REST API complies with the Cache constraint by thoughtfully employing various HTTP response headers, including *Cache-Control*, *E-Tag*, *Last-Modified* and *Vary* to distinctly mark responses as (non-)cacheable.
...
4. Uniform Interface
   ...
   1. identification of resources
       ...
       **UDM REST API Compliance**: The *UDM REST API* conforms to this by assigning unique URLs to each unique resource, ensuring clear and consistent identification.
   ...
   2. manipulation of resources through representations
       ...
       **UDM REST API Compliance**: The *UDM REST API* adheres to this constraint by providing diverse representations of individual resources.
   ...
   4. hypermedia as the engine of application state (HATEOAS)
       ...
       **UDM REST API Compliance**: The *UDM REST API* demonstrates compliance with various aspects of HATEOAS. It provides links in the HTTP Link response header and includes links and embedded resources in the HAL+JSON responses.
...
5. Layered System
    ...
    **UDM REST API Compliance**: The *UDM REST API* conforms to this by operating in a stateless manner, ensuring that each request carries all necessary information for understanding and processing.
...
They are all wrong and come withing severe disadvantages and maintainability problems (you can research this on your own, there are lot of articles).
Versioning also has nothing to do with *REST*, the words isn't even mentioned in its definition.
On the contrary the above versioning approaches makes services unRESTful.
...
Best practices for dealing with changes without versioning:
