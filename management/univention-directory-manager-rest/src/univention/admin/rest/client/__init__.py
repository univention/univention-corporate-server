...
class Object(Client):
    ...
    def _modify(self, reload=True):
        ...
        response = self.client.make_request('PUT', self.uri, data=self.representation, allow_redirects=False, custom_redirect_handling=True, **headers)  # type: ignore # <https://github.com/python/mypy/issues/10008>
    ...
    def _create(self, reload=True):
        ...
        response = self.client.make_request('POST', uri['href'], data=self.representation, allow_redirects=False, custom_redirect_handling=True)
        self._reload_from_response(response, reload)
        return response
