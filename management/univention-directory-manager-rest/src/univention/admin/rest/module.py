...
class Resource(ResourceBase, RequestHandler):
    ...
    def handle_udm_errors(self, action, *args, **kwargs):
        try:
            ...
            if isinstance(exc.original_exception, ldap.TYPE_OR_VALUE_EXISTS | ldap.NO_SUCH_ATTRIBUTE):
                raise ServerError(_('The object was modified by another request at the same time. Please retry the request.'))
...
class Metrics(Resource):
    ...
    def get(self):
        ...
        self.license_limit_users.labels(domain=domain, platform=platform, license_uuid=license_key).set(limit)
...
class Application(tornado.web.Application):
    ...
    def __init__(self, **settings):
        ...
        super().__init__([
            ...
            (rf"{root_path}/udm/((?:css|js|img|schema|swaggerui)/.*)", tornado.web.StaticFileHandler, {"path": str(Path(__file__).parent / "www"), "default_filename": "index.html"}),
