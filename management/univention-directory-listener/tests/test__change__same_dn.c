...
TEST(semicolon,
	"cn=foo,dc=univention,dc=de",
	"cn=foo;dc=univention;dc=de", BUG true);
...
TEST(oid,
	"2.5.4.3=foo,dc=univention,dc=de",
	"oid.2.5.4.3=foo;dc=univention;dc=de", BUG true);
TEST(OID,
	"2.5.4.3=foo,dc=univention,dc=de",
	"OID.2.5.4.3=foo;dc=univention;dc=de", BUG true);
// Implementations MUST allow a value to be surrounded by quote ('"' ASCII 34) characters
TEST(quoted,
	"cn=foo\\,bar,dc=univention,dc=de",
	"cn=\"foo bar\",dc=univention,dc=de", BUG true);

TEST(octothorpe,
	"cn=foo,dc=univention,dc=de",
	"cn=#666f6f,dc=univention,dc=de", false); // LDAP_AVA_STRING != LDAP_AVA_BINARY
