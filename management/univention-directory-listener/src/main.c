...
static void usage(void) {
	fprintf(stderr, "Usage: univention-directory-listener -h <host> -m <path> [-x -D <dn> (-w <pw> | -y <file>)] [options]\n\n");
	fprintf(stderr, "Options:\n");
	fprintf(stderr, "   -d <level>  set debug level 0-4\n");
	fprintf(stderr, "   -F          run in foreground (intended for process supervision)\n");
	fprintf(stderr, "   -h <host>   UDN (and LDAP) server address\n");
	fprintf(stderr, "   -H <uri>    LDAP server URI\n");
	fprintf(stderr, "   -p <port>   LDAP server port\n");
	fprintf(stderr, "   -b <dn>     LDAP base dn\n");
	fprintf(stderr, "   -x          LDAP simple bind\n");
	fprintf(stderr, "   -D <dn>     LDAP bind dn\n");
	fprintf(stderr, "   -w <pw>     LDAP bind password\n");
	...
	fprintf(stderr, "   -U <user>   SASL username\n");
