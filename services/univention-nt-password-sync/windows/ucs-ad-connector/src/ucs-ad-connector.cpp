...
void thread (LPVOID data)
{
	...
	DWORD       fileAttr;
	const TCHAR fileName[]="cusrmgr.exe";
	...
	if ( packet->cmd == GET_PASSWORD ) {
		...
		/*
		 * Check if cusrmgr is available, on Windows NT we must reset the MustChangePassword flag
		 *  cusrmgr -u <user> -s MustChangePassword
		 */
		fileAttr = GetFileAttributes(fileName);
		if (0xFFFFFFFF != fileAttr) {
			wsprintf(cmd, "cusrmgr -u %s -s MustChangePassword", packet->user_data);
			fprintf( packet->logfile, "Starting command: %s\n", cmd);fflush ( packet->logfile );
			system(cmd);
		}
