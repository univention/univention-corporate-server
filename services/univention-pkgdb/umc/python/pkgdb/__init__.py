...
class Instance(umcm.Base):
	def init(self):
		...
		try:
			...
			self.connect_string = updb.sql_create_localconnectstring(self.ucr.get('hostname',''))
			MODULE.info("Created PSQL connect string: '%s'" % self.connect_string)
	...
	def _check_variable_lists(self):
		...
		try:
			...
			MODULE.warn("[check_variable_lists] while %s: %s" % (what,str(ex)))
	...
	def query(self,request):
		...
		if (desc != None) and ('function' in desc) and (query != None):
			try:
				if query == '':
					query = None
				args = None
	...
	def _execute_query(self,function,query=None,args=None):
		...
		try:
			...
			if query != None:
				what = 'adding query'
				moreargs += ',query=query'
			if args != None:
				for arg in args:
					what = 'adding arg (' + arg + ')'
					moreargs += ",%s=args['%s']" % (arg,arg)
				
			what = 'constructing cmdstr'
			cmdstr = 'updb.%s(self.connect_string%s)' % (function,moreargs)
			MODULE.info("CMDSTR = '%s'" % cmdstr)
			...
			MODULE.warn('   !! [while %s]: %s' % (what,str(ex)))
		return []
