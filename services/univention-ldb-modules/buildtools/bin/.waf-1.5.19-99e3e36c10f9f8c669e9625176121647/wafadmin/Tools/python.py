...
def check_python_version(conf,minver=None):
	...
	if result:
		...
		if'PYTHONARCHDIR'in conf.environ:
			...
			(pyarchdir,)=_get_python_variables(python,["get_python_lib(plat_specific=1, standard_lib=0, prefix=%r)"%conf.env['PREFIX']],['from distutils.sysconfig import get_config_var, get_python_lib'])
			if not pyarchdir:
				pyarchdir=pydir
...
before('apply_incpaths','apply_lib_vars','apply_type_vars','apply_bundle')(init_pyext)
after('vars_target_cshlib')(init_pyext)
