/*
 * Univention Bind
 *  DLZ-dlopen LDAP backend for BIND 9.20+
 *
 * Loaded via /etc/bind/named.conf:
 *   dlz "ucs-ldap" {
 *       database "dlopen /usr/lib/bind9/ucs-ldapdb.so <url> <ttl>";
 *       search yes;
 *   };
 *
 * Built against the vendored ISC dlz_minimal.h (see that file for its
...
#define LDAPDB_PUTRR(lookup, type, ttl, data) \
    (dlz_putrr_cb != NULL ? dlz_putrr_cb((lookup), (type), (ttl), (data)) : ISC_R_FAILURE)
#define LDAPDB_PUTNAMEDRR(all, name, type, ttl, data) \
    (dlz_putnamedrr_cb != NULL ? dlz_putnamedrr_cb((all), (name), (type), (ttl), (data)) : ISC_R_FAILURE)
...
static void
format_attrs(char **attrs, char *buf, size_t buflen) {
...
static void
ldapdb_log(int level, const char *fmt, ...) {
    char buf[2048];
    va_list ap;
...
static int
parseextensions(char *extensions, ldapdb_data_t *data) {
...
static void
ldapdb_free_data(ldapdb_data_t *data) {
    if (data == NULL) {
        return;
    }
...
static ldapdb_thread_state_t *
find_thread_state(ldapdb_data_t *data, pthread_t tid) {
...
static LDAP **
ldapdb_getconn(ldapdb_data_t *data) {
...
static bool
ldapdb_bind(ldapdb_data_t *data, LDAP **ldp) {
    ...
    for (int attempt = 1; attempt <= 3; attempt++) {
        ...
        if (rc != LDAP_SUCCESS) {
            ldapdb_log(ISC_LOG_ERROR, "ldapdb_dlz: ldap_initialize(%s) failed: %s",
                       data->url, ldap_err2string(rc));
            sleep(5);
            continue;
        }
        ...
        if (data->tls) {
            ...
            if (rc != LDAP_SUCCESS) {
                ldapdb_log(ISC_LOG_ERROR, "ldapdb_dlz: ldap_start_tls_s() failed: %s",
                           ldap_err2string(rc));
                sleep(5);
                continue;
            }
        ...
        rc = ldap_sasl_bind_s(*ldp, data->bindname, LDAP_SASL_SIMPLE,
                              &cred, NULL, NULL, NULL);
        if (rc == LDAP_SUCCESS) {
            return true;
        }

        ldapdb_log(ISC_LOG_ERROR, "ldapdb_dlz: bind as '%s' failed: %s",
                   data->bindname != NULL ? data->bindname : "<anonymous>",
                   ldap_err2string(rc));
        sleep(5);
    }
...
static isc_result_t
append_rr(ldapdb_rrset_t **head, const char *owner, const char *type,
          unsigned int ttl, const char *rdata) {
...
static isc_result_t
ldapdb_collect(const char *zone, const char *name, ldapdb_data_t *data,
               ldapdb_rrset_t **out) {
    ...
    *out = NULL;
    ...
    if (name == NULL) {
        ...
        snprintf(data->filtername,
                 data->filteronelen - (size_t)(data->filtername - data->filterone),
                 "%s))", name);
        fltr = data->filterone;
    }

    {
        ...
        ldapdb_log(ISC_LOG_INFO,
                   "ldapdb_dlz: ldap_search base='%s' scope=%s filter='%s' attrs=%s",
                   data->base != NULL ? data->base : "",
                   scope_str(data->scope), fltr, attrs_buf);
    }

    for (int retry = 0; retry < 2; retry++) {
        rc = ldap_search_ext_s(*ldp, data->base, data->scope, fltr,
                               data->attrs, 0, NULL, NULL, NULL, 0, &res);
        if (rc == LDAP_SUCCESS) {
            break;
        }
        ...
        ldapdb_log(ISC_LOG_ERROR, "ldapdb_dlz: ldap_search_ext_s zone=%s name=%s filter=%s failed: %s",
                   zone, name != NULL ? name : "<allnodes>", fltr,
                   ldap_err2string(rc));
        return ISC_R_FAILURE;
    }
    ...
    ldapdb_log(ISC_LOG_INFO,
               "ldapdb_dlz: ldap_search filter='%s' -> %d entries",
               fltr, ldap_count_entries(*ldp, res));
    ...
    for (e = ldap_first_entry(*ldp, res); e != NULL; e = ldap_next_entry(*ldp, e)) {
        ...
        for (char *a = ldap_first_attribute(*ldp, e, &ptr);
             a != NULL;
             a = ldap_next_attribute(*ldp, e, ptr)) {
            char *record_suffix;
            size_t typelen;
                    ...
                    if (name != NULL) {
                        ...
                        for (int j = 0; names[j] != NULL && names[j]->bv_val != NULL; j++) {
                            ...
                            if (owner[0] == LDAPDB_WILDCARD_INT[0]) {
                                snprintf(ownerbuf, sizeof(ownerbuf), "%s%s",
                                         LDAPDB_WILDCARD_EXT, owner + 1);
                                owner = ownerbuf;
                            }
...
static isc_result_t
ldapdb_lookup_name(const char *zone, const char *name, ldapdb_data_t *data,
...
static isc_result_t
ldapdb_allnodes_emit(const char *zone, ldapdb_data_t *data,
 ...
 */
static bool
has_descendants(ldapdb_data_t *data, const char *name) {
    LDAP **ldp;
    ...
    ldapdb_log(ISC_LOG_INFO,
               "ldapdb_dlz: ldap_search base='%s' scope=%s filter='%s' attrs=1.1 sizelimit=1 (descendant-probe)",
               data->base != NULL ? data->base : "",
               scope_str(data->scope), data->filterone);

    rc = ldap_search_ext_s(*ldp, data->base, data->scope, data->filterone,
                           no_attrs, 1, NULL, NULL, NULL,
                           1 /* sizelimit */, &res);
    if (rc == LDAP_SUCCESS || rc == LDAP_SIZELIMIT_EXCEEDED) {
        count = ldap_count_entries(*ldp, res);
    } else {
        ldapdb_log(ISC_LOG_INFO,
                   "ldapdb_dlz: descendant-probe filter='%s' failed: %s",
                   data->filterone, ldap_err2string(rc));
    }
    if (res != NULL) {
        ldap_msgfree(res);
    }
    ldapdb_log(ISC_LOG_INFO,
               "ldapdb_dlz: ldap_search filter='%s' -> %d match (descendant-probe)",
               data->filterone, count);
    return count > 0;
}

static isc_result_t
parse_url_config(char *url, ldapdb_data_t *data) {
...
static isc_result_t
set_zone_filters(ldapdb_data_t *data, const char *zone) {
    ...
    if (asprintf(&new_all, "%.*s%s%s",
                 (int)(p - data->filterall_template),
                 data->filterall_template, zone, p + 2) < 0) {
        return ISC_R_NOMEMORY;
    }
    ...
    if (asprintf(&new_one_prefix, "%.*s%s%s",
                 (int)(p - data->filterone_template),
                 data->filterone_template, zone, p + 2) < 0) {
        free(new_all);
        return ISC_R_NOMEMORY;
    }
...
dlz_create(const char *dlzname, unsigned int argc, char *argv[], void **dbdata, ...) {
     ...
     */
    if (argc < 3) {
        ldapdb_log(ISC_LOG_ERROR,
                   "ldapdb_dlz: expected LDAP URL and default TTL after driver path (argc=%u)",
                   argc);
        return ISC_R_FAILURE;
    }
    ...
    if (data->defaultttl == 0) {
        ldapdb_log(ISC_LOG_ERROR,
                   "ldapdb_dlz: invalid default TTL '%s'", argv[2]);
...
dlz_findzonedb(void *dbdata, const char *name,
    ...
    if (result != ISC_R_SUCCESS) {
        ldapdb_log(ISC_LOG_INFO,
                   "ldapdb_dlz: findzonedb(%s): set_zone_filters failed (%u)",
                   name, result);
        return result;
    }
    ...
    rrset_free(rrs);
    ldapdb_log(ISC_LOG_INFO,
               "ldapdb_dlz: findzonedb(%s) filter=%s -> %s",
               name, data->filterall,
               result == ISC_R_SUCCESS ? "FOUND" : "NOTFOUND");
...
dlz_lookup(const char *zone, const char *name, void *dbdata,
     ...
     */
    ldapdb_log(ISC_LOG_INFO,
               "ldapdb_dlz: lookup(zone=%s relativeDomainName=%s)", zone, name);
    ...
    if (result == ISC_R_NOTFOUND &&
        ...
        has_descendants(data, name)) {
        ldapdb_log(ISC_LOG_INFO,
                   "ldapdb_dlz: lookup(zone=%s name=%s) intermediate node, descending",
                   zone, name);
        return ISC_R_SUCCESS;
    }
    ...
    if (result == ISC_R_NOTFOUND && strcmp(name, "@") != 0 && data->wildcard) {
        ...
        while (result == ISC_R_NOTFOUND && (np = strchr(np + 1, '.')) != NULL) {
            snprintf(srch_name, strlen(name) + strlen(LDAPDB_WILDCARD_INT) + 1,
                     "%s%s", LDAPDB_WILDCARD_INT, np);
