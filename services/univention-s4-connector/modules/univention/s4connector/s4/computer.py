...
def windowscomputer_sync_s4_to_ucs_check_rename(s4connector, key, sync_object):
    ud.debug(ud.LDAP, ud.INFO, f"con_check_rename: sync_object: {sync_object}")
    ...
    try:
        sAMAccountName_vals = next(_v for _k, _v in attrs.items() if _k.lower() == 'samaccountname')
    except IndexError:
        raise ValueError(f"{sync_object['dn']} has no sAMAccountName")
    else:
        sAMAccountName = sAMAccountName_vals[0]

    ucs_object = s4connector.get_ucs_ldap_object(sync_object['dn'])
    if not ucs_object:
        ud.debug(ud.LDAP, ud.WARN, f"con_check_rename: ucs object not found: {sync_object['dn']} (maybe already deleted)")
        return
    ud.debug(ud.LDAP, ud.INFO, f"con_check_rename: ucs object: {ucs_object}")
    ...
    ud.debug(ud.LDAP, ud.PROCESS, f"con_check_rename: Renaming client from {ucs_uid} to {sAMAccountName}")
