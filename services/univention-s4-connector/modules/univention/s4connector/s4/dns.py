...
def dns_dn_mapping(s4connector, given_object, dn_mapping_stored, isUCSobject):
    ...
    if obj['dn'] is not None:
        try:
            s4_RR_val = next(_value for _key, _value in obj['attributes'].items() if s4_RR_attr.lower() == _key.lower())[0].decode('UTf-8')
        ...
        if (dn_key not in dn_mapping_stored) or (not given_object[dn_key]):
            ...
            if isUCSobject:
                ...
                if premapped_dn is not None:
                    ...
                    ud.debug(ud.LDAP, ud.INFO, f"dns_dn_mapping: premapped S4 object: {premapped_dn}")
                    return True
                else:
                ...
                if premapped_dn is not None:
                    ...
                    ud.debug(ud.LDAP, ud.INFO, f"dns_dn_mapping: premapped UCS object: {premapped_dn}")
                    return True
                else:
    ...
    for dn_key in ['dn', 'olddn']:
        ud.debug(ud.LDAP, ud.INFO, f"dns_dn_mapping: check newdn for key '{dn_key}'")
        if dn_key in obj and not dn_premapped(obj, dn_key, dn_mapping_stored):

            dn = obj[dn_key]
            ud.debug(ud.LDAP, ud.INFO, f"dns_dn_mapping: dn: {dn}")
            ...
            if isUCSobject:
                ...
                if relativeDomainName == '@':  # or dn starts with 'zoneName='
                    ...
                    ud.debug(ud.LDAP, ud.INFO, f"dns_dn_mapping: get dns_dn_mapping for target zone {target_zone_dn}")
                    fake_ol_zone_object = {
                        'dn': target_zone_dn,
                        'attributes': {
                    ...
                    ud.debug(ud.LDAP, ud.INFO, f"dns_dn_mapping: search in S4 base {s4_zone_dn}")
                ...
                if s4dn_utf16_le:  # no referral, so we've got a valid result
                    s4dn = s4dn_utf16_le
                    ud.debug(ud.LDAP, ud.INFO, f"dns_dn_mapping: got s4dn {s4dn}")
                    if dn_key == 'olddn' or (dn_key == 'dn' and 'olddn' not in obj):
                        ...
                        ud.debug(ud.LDAP, ud.INFO, f"dns_dn_mapping: move case newdn={newdn}")
                else:
                ...
                while not s4_RR_val:  # in case of olddn this is already set
                    ...
                    try:
                        ...
                        ud.debug(ud.LDAP, ud.INFO, f"dns_dn_mapping: got {s4_RR_attr} from S4")

                for ucsval, conval in s4connector.property[propertyname].mapping_table.get(propertyattrib, []):
                    if s4_RR_val.lower() == conval.lower():
                        s4_RR_val = ucsval
                        ud.debug(ud.LDAP, ud.INFO, f"dns_dn_mapping: map {s4_RR_attr} according to mapping-table")
                        continue
                ...
                if b'dnsZone' in s4_ocs:
                    ...
                    ud.debug(ud.LDAP, ud.INFO, f"dns_dn_mapping: get dns_dn_mapping for {target_zone_dn}")
                ...
                ud.debug(ud.LDAP, ud.INFO, f"dns_dn_mapping: UCS filter: {s4_filter}")
                ud.debug(ud.LDAP, ud.INFO, f"dns_dn_mapping: UCS base: {base}")
                try:
                ...
                ud.debug(ud.LDAP, ud.ALL, f"dns_dn_mapping: Found ucsdn: {ucsdn}")
...
def __create_s4_forward_zone(s4connector, zone_dn):
    ...
    ud.debug(ud.LDAP, ud.INFO, f'_dns_zone_forward_con_create: dn: {zone_dn}')
    ud.debug(ud.LDAP, ud.INFO, f'_dns_zone_forward_con_create: al: {al}')
...
def __create_s4_dns_node(s4connector, dnsNodeDn, relativeDomainNames, dnsRecords):
    ...
    ud.debug(ud.LDAP, ud.INFO, f'__create_s4_dns_node: dn: {dnsNodeDn}')
    ud.debug(ud.LDAP, ud.INFO, f'__create_s4_dns_node: al: {al}')
...
def __pack_mxRecord(object, dnsRecords):
    for mXRecord in object['attributes'].get('mXRecord', []):
        if mXRecord:
            ud.debug(ud.LDAP, ud.INFO, f'__pack_mxRecord: {mXRecord}')
            mx = mXRecord.split(b' ')
            priority = mx[0]
            name = mx[1]
            mx_record = MXRecord(name, int(priority))
            dnsRecords.append(ndr_pack(mx_record))
            ud.debug(ud.LDAP, ud.INFO, f'__pack_mxRecord: {ndr_pack(mx_record)}')
...
def __pack_txtRecord(object, dnsRecords):
    for txtRecord in object['attributes'].get('tXTRecord', []):
        if txtRecord:
            ud.debug(ud.LDAP, ud.INFO, f'__pack_txtRecord: {txtRecord}')
            ...
            ud.debug(ud.LDAP, ud.INFO, f'__pack_txtRecord: {ndr_txt_record}')
...
def __get_s4_msdcs_soa(s4connector, zoneName):
    ...
    for base in s4connector.s4_ldap_partitions:
        ...
        ud.debug(ud.LDAP, ud.WARN, f"__get_s4_msdcs_soa: _msdcs sub-zone for {zoneName} not found in S4")
        return
...
def s4_zone_msdcs_sync(s4connector, object):
    ...
    soaRecord = object['attributes'].get('sOARecord', [None])[0]
    if not soaRecord:
        ud.debug(ud.LDAP, ud.WARN, f's4_zone_msdcs_sync: OL zone {domainZoneName} has no SOA info')
        return
    ...
    for dnsRecord in old_dnsRecords:
        ...
        if ndrRecord.wType == dnsp.DNS_TYPE_SOA:
            if ndrRecord.data.serial >= serial:
                ud.debug(ud.LDAP, ud.WARN, f's4_zone_msdcs_sync: SOA serial OpenLDAP zone {domainZoneName} is higher than corresponding value of {msdcs_soa_dn}')
                return
    ...
    if not found:
        ud.debug(ud.LDAP, ud.WARN, f's4_zone_msdcs_sync: object {msdcs_soa_dn} has no SOA info')
        return
...
def ucs_host_record_create(s4connector, object):
    ud.debug(ud.LDAP, ud.INFO, f'ucs_host_record_create: object: {object}')
    ...
    if len(searchResult) > 0:
        ...
        ud.debug(ud.LDAP, ud.INFO, f'ucs_host_record_create: zoneDN: {zoneDN}')
...
def ucs_host_record_delete(s4connector, object):
    ud.debug(ud.LDAP, ud.INFO, f'ucs_host_record_delete: object: {object}')
    ...
    if len(searchResult) > 0:
        ...
        ud.debug(ud.LDAP, ud.INFO, f'ucs_host_record_delete: Object was not found, filter was: {ol_filter}')
...
def ucs_ptr_record_create(s4connector, object):
    ud.debug(ud.LDAP, ud.INFO, f'ucs_ptr_record_create: object: {object}')
...
def ucs_ptr_record_delete(s4connector, object):
    ud.debug(ud.LDAP, ud.INFO, f'ucs_ptr_record_delete: object: {object}')
    ...
    if len(searchResult) > 0:
        ...
        ud.debug(ud.LDAP, ud.INFO, f'ucs_ptr_record_delete: Object was not found, filter was: {ol_filter}')

    return True


def ucs_cname_create(s4connector, object):
    ud.debug(ud.LDAP, ud.INFO, f'ucs_cname_create: object: {object}')
...
def ucs_cname_delete(s4connector, object):
    ud.debug(ud.LDAP, ud.INFO, f'ucs_cname_delete: object: {object}')
    ...
    if len(searchResult) > 0:
        ...
        ud.debug(ud.LDAP, ud.INFO, f'ucs_cname_delete: Object was not found, filter was: {ol_filter}')
...
def ucs_srv_record_create(s4connector, object):
    ud.debug(ud.LDAP, ud.INFO, f'ucs_srv_record_create: object: {object}')
    ...
    ud.debug(ud.LDAP, ud.INFO, f'ucs_srv_record_create: ucr_locations for connector/s4/mapping/dns/srv_record/{relativeDomainName.lower()}.{zoneName.lower()}/location: {ucr_locations}')
    ...
    if len(searchResult) > 0:
        ...
        if ucr_locations:
            ...
            ud.debug(ud.LDAP, ud.INFO, f'ucs_srv_record_create: location: {newRecord["location"]}')
            ud.debug(ud.LDAP, ud.INFO, f'ucs_srv_record_create: srv     : {srv}')
            srv.sort()
...
def ucs_srv_record_delete(s4connector, object):
    ud.debug(ud.LDAP, ud.INFO, f'ucs_srv_record_delete: object: {object}')
    ...
    if len(searchResult) > 0:
        ...
        ud.debug(ud.LDAP, ud.INFO, f'ucs_srv_record_delete: Object was not found, filter was: {ol_filter}')
...
def s4_srv_record_create(s4connector, object):
    ...
    # ucr set connector/s4/mapping/dns/srv_record/_ldap._tcp.test.example/location='100 0 389 foobar.test.example. 100 0 389 foobar2.test.example.'
    ucr_locations = s4connector.configRegistry.get(f'connector/s4/mapping/dns/srv_record/{relativeDomainName.lower()}.{zoneName.lower()}/location')
    ud.debug(ud.LDAP, ud.INFO, f's4_srv_record_create: ucr_locations for connector/s4/mapping/dns/srv_record/{relativeDomainName.lower()}.{zoneName.lower()}/location: {ucr_locations}')
    if ucr_locations:
...
def ucs_txt_record_create(s4connector, object):
    ud.debug(ud.LDAP, ud.INFO, f'ucs_txt_record_create: object: {object}')
    udm_property = 'txt'
...
def ucs_txt_record_delete(s4connector, object):
    ud.debug(ud.LDAP, ud.INFO, f'ucs_txt_record_delete: object: {object}')
    ...
    if len(searchResult) > 0:
        ...
        ud.debug(ud.LDAP, ud.INFO, f'ucs_txt_record_delete: Object was not found, filter was: {ol_filter}')
...
def ucs_ns_record_create(s4connector, object):
    ud.debug(ud.LDAP, ud.INFO, f'ucs_ns_record_create: object: {object}')
...
def ucs_ns_record_delete(s4connector, object):
    ud.debug(ud.LDAP, ud.INFO, f'ucs_ns_record_delete: object: {object}')
    ...
    if len(searchResult) > 0:
        ...
        ud.debug(ud.LDAP, ud.INFO, f'ucs_ns_record_delete: Object was not found, filter was: {ol_filter}')
...
def ucs_zone_create(s4connector, object, dns_type):
    ...
    if relativeDomainName != '@':
        ud.debug(ud.LDAP, ud.INFO, f"ucs_zone_create: ignoring DC={relativeDomainName} object")
        return
...
def ucs_zone_delete(s4connector, object, dns_type):
    ...
    if relativeDomainName != '@':
        ud.debug(ud.LDAP, ud.INFO, f"ucs_zone_delete: ignoring DC={relativeDomainName} object")
        return
...
def ucs2con(s4connector, key, object):
    ...
    if not dns_type:
        ...
        ud.debug(ud.LDAP, ud.INFO, f'dns ucs2con: Ignore unknown dns object: {object["dn"]}')
        return True

    ud.debug(ud.LDAP, ud.INFO, f'dns ucs2con: Object ({object["dn"]}) is of type {dns_type}')
...
def con2ucs(s4connector, key, object):
    ...
    ud.debug(ud.LDAP, ud.INFO, f'dns con2ucs: Object ({object["dn"]}): {object}')
    ...
    if not dns_type:
        # unknown object -> ignore
        ud.debug(ud.LDAP, ud.INFO, f'dns con2ucs: Ignore unknown dns object: {object["dn"]}')
        return True

    ud.debug(ud.LDAP, ud.INFO, f'dns con2ucs: Object ({object["dn"]}) is of type {dns_type}')
