...
def password_sync_ucs_to_s4(connector, key, object):
	_d=ud.function('ldap.ad.password_sync_ucs_to_s4')
	...
	try:
		res = connector.lo.lo.search(base=ucs_object['dn'], scope='base', attr=['sambaLMPassword', 'sambaNTPassword','sambaPwdLastSet','sambaPwdMustChange', 'krb5PrincipalName', 'krb5Key', 'shadowLastChange', 'shadowMax', 'krb5PasswordEnd'])
	except ldap.NO_SUCH_OBJECT:
	...
	pwd_set = False
	# pwd_ad_res = get_password_from_s4(connector, rid)
	res=connector.lo_ad.lo.search_s(connector.lo_ad.base, ldap.SCOPE_SUBTREE, compatible_modstring('(objectSid=%s)' % objectSid), ['unicodePwd', 'supplementalCredentials', 'msDS-KeyVersionNumber', 'dBCSPwd'])
	...
	if modlist:
		connector.lo_ad.lo.modify_ext_s(compatible_modstring(object['dn']), modlist, serverctrls=[ ctrl_bypass_password_hash ])


def password_sync_s4_to_ucs(connector, key, ucs_object):
	_d=ud.function('ldap.ad.password_sync_s4_to_ucs')
	...
	if res[0][1].has_key('pwdLastSet'):
		pwdLastSet = long(res[0][1]['pwdLastSet'][0])
	ud.debug(ud.LDAP, ud.INFO, "password_sync_s4_to_ucs: pwdLastSet from AD: %s (%s)" % (pwdLastSet,res))
	objectSid = univention.connector.ad.decode_sid(res[0][1]['objectSid'][0])
	...
	# res = get_password_from_ad(connector, rid)
	res=connector.lo_ad.lo.search_s(connector.lo_ad.base, ldap.SCOPE_SUBTREE, univention.connector.ad.compatible_modstring('(objectSid=%s)' % objectSid), ['unicodePwd', 'supplementalCredentials', 'msDS-KeyVersionNumber', 'dBCSPwd'])
	unicodePwd_attr = res[0][1].get('unicodePwd', [None])[0]
	if unicodePwd_attr:
		...
		userPassword = ''
		modlist=[]
		res=connector.lo.search(base=ucs_object['dn'], attr=['sambaPwdMustChange', 'sambaPwdLastSet','sambaNTPassword', 'sambaLMPassword', 'krb5PrincipalName', 'krb5Key', 'krb5KeyVersionNumber', 'userPassword', 'shadowLastChange', 'shadowMax', 'krb5PasswordEnd'])
