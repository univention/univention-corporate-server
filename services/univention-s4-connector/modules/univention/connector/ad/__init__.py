...
class ad(univention.connector.ucs):
	def __init__(self, CONFIGBASENAME, property, baseConfig, ad_ldap_host, ad_ldap_port, ad_ldap_base, ad_ldap_binddn, ad_ldap_bindpw, ad_ldap_certificate, listener_dir):
		...
		protocol = baseConfig.get('%s/ad/ldap/protocol' % CONFIGBASENAME, 'ldap').lower()
		if protocol == 'ldapi':
			import urllib
			socket = urllib.quote(baseConfig.get('%s/ad/ldap/socket' % CONFIGBASENAME, ''), '')
			ldapuri = "%s://%s" % (protocol, socket)
		else:
			ldapuri = "%s://%s:%d" % (protocol, baseConfig['%s/ad/ldap/host' % CONFIGBASENAME],int(baseConfig['%s/ad/ldap/port' % CONFIGBASENAME]))
		...
		try:
			...
			res = self.lo_ad.lo.search_ext_s('',ldap.SCOPE_BASE, 'objectclass=*',[],
