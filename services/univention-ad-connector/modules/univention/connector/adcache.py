            ...
            try:
                ...
                for sql_command in sql_commands:
                    if isinstance(sql_command, tuple):
                        log.trace("ADCache: Execute SQL command: '%s', '%s'", sql_command[0], sql_command[1])
                        cur.execute(sql_command[0], sql_command[1])
                    else:
                        log.trace("ADCache: Execute SQL command: '%s'", sql_command)
                ...
                cur.close()
                if fetch_result:
                    log.trace("ADCache: Return SQL result: '%s'", rows)
                    return rows
                return None
            except sqlite3.Error as exp:
                log.warning("ADCache: sqlite: %s. SQL command was: %s", exp, sql_commands)
                if self._dbcon:
                    self._dbcon.close()
                self._dbcon = sqlite3.connect(self.filename)
