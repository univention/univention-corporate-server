...
class ad(univention.connector.ucs):
    ...
    def __init__(self, CONFIGBASENAME, property, configRegistry, ad_ldap_host, ad_ldap_port, ad_ldap_base, ad_ldap_binddn, ad_ldap_bindpw, ad_ldap_certificate, listener_dir, max_retry_rejected, logfilename=None, debug_level=None):
        ...
        if not self.config.has_section('AD rejected reason'):
            ud.debug(ud.LDAP, ud.INFO, "__init__: init add config section 'AD rejected reason'")
    ...
    def check_syncmode_ad(self, property_key, ad_object):
        if not property_key or self.property[property_key].sync_mode in ['write', 'none']:
            if property_key:
                ud.debug(ud.LDAP, ud.INFO, "sync_to_ucs ignored, sync_mode is %s" % self.property[property_key].sync_mode)
            else:
    ...
    def __get_object_changes_ad(self, ad_object):
        ...
        ud.debug(ud.LDAP, ud.INFO, "The following attributes have been changed: %s" % ad_object['changed_attributes'])
        return ad_object
        ...
        for change_usn, dn, retry_count in rejected:
            ...
            try:
                ...
                if not elements or len(elements) < 1 or not elements[0][0]:
                    ...
                    if not ad_object:
                        ud.debug(ud.LDAP, ud.INFO, "sync to ucs: Nothing todo, drop reject %s" % (dn,))
                        self._remove_rejected(change_usn)
                        continue
    ...
    def poll(self, show_deleted=True):
        ...
        for element in changes:
            ...
            if not ad_object:
                print_progress(True)
                if self.profiling:
                    ud.debug(ud.LDAP, ud.PROCESS, "POLL FROM CON: Processed non-change of %s" % (element[0],))
                continue
