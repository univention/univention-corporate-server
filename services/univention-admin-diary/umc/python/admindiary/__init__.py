...
class Instance(Base):
	def init(self):
		# TODO: what if no backend is defined?
		# TODO: dedicated ucr variable
		# TODO: more than one server
		ucr = ConfigRegistry()
		ucr.load()
		server = ucr.get('admin/diary/backend').split()[0]
		self.backend = 'https://%s/admindiary/' % server
		username = ucr.get('ldap/hostdn')
		password = open('/etc/machine.secret').read()
		self.http_auth = HTTPBasicAuth(username, password)
		self.translations = {}

	def _translate(self, event_name):
		if event_name not in self.translations:
			# TODO: auth!
			response = requests.get(self.backend + 'translate', json={'event': event_name, 'locale': self.locale.language})
			translation = response.json()
			self.translations[event_name] = translation
		return self.translations[event_name]

	def _format_entry(self, entry):
		message = entry['message']
		if entry['event'] != 'COMMENT':
			message = self._translate(entry['event'])
		try:
		...
		icon = 'default'
		event = DiaryEvent.get(entry['event'])
		if event:
	...
	def options(self):
		# TODO: auth!
		response = requests.get(self.backend + 'options')
		return response.json()

	@simple_response
	def get(self, context_id):
		with get_client(version=2) as client:
	...
	def query(self, time_from=None, time_until=None, tag=None, event=None, username=None, hostname=None, message=None):
		...
		response = requests.get(self.backend + 'api/v2/entries/', json=args, auth=self.http_auth)
	...
	def add_comment(self, context_id, message):
		...
		# TODO: 1 second is not enough, yet more looks wrong
